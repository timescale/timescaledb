...
if [ ! -d "${BUILD_DIR}" ]; then
  ...
  # FIXME
  BUILD_DIR="${BUILD_DIR}" ./bootstrap -DREAD_LEGACY_BLOOM1_INDEX=1 -DCMAKE_BUILD_TYPE=Release -DWARNINGS_AS_ERRORS=OFF -DASSERTIONS=ON -DLINTER=OFF -DGENERATE_DOWNGRADE_SCRIPT=ON -DREGRESS_CHECKS=OFF -DTAP_CHECKS=OFF
fi
