...
-- A ddl_command_start handler that raises aborts the INSERT.
CREATE OR REPLACE FUNCTION raise_pub_error() RETURNS event_trigger AS $$
BEGIN
    RAISE EXCEPTION 'blocked by ddl_command_start handler';
END;
$$ LANGUAGE plpgsql;

CREATE EVENT TRIGGER pub_block ON ddl_command_start
    WHEN TAG IN ('ALTER PUBLICATION') EXECUTE FUNCTION raise_pub_error();

\set ON_ERROR_STOP 0
INSERT INTO pub_test VALUES ('2024-01-05 00:00:00+00', 5, 5.0);
\set ON_ERROR_STOP 1

-- The aborted INSERT rolled the new chunk back.
SELECT count(*) AS chunks FROM show_chunks('pub_test');

DROP EVENT TRIGGER pub_block;

-- The retry leaves no stale state: one ddl_command_end per publication.
INSERT INTO pub_test VALUES ('2024-01-05 00:00:00+00', 5, 5.0);
SELECT count(*) AS chunks FROM show_chunks('pub_test');
