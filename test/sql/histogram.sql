...
-- standard 1 bucket with different aggregates
SELECT histogram(key, 1, 3, 1, 'avg') FROM hitest1;
SELECT histogram(key, 1, 3, 1, 'max') FROM hitest1;
SELECT histogram(key, 1, 3, 1, 'min') FROM hitest1;
SELECT histogram(key, 1, 3, 1, 'sum') FROM hitest1;
SELECT histogram(key, 1, 3, 1, 'count') FROM hitest1;

\set ON_ERROR_STOP 0
-- lower bound cannot be > upper bound
SELECT histogram(key, 3, 1, 1) FROM hitest1;
\set ON_ERROR_STOP 1
...
SELECT qualify, histogram(score, 0, 10, 5, 'avg') FROM hitest2 GROUP BY qualify;
