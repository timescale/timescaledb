...
-- Test enable_hypertable_expansion_for_dml GUC.
-- With our expansion, UPDATE hits UUID v7 validation before the chunk
-- constraint. With GUC off, PG's own expansion hits the constraint first.
CREATE TABLE uuid_guc_test(id uuid primary key);
SELECT create_hypertable('uuid_guc_test', 'id', chunk_time_interval => interval '1 day');
BEGIN;
INSERT INTO uuid_guc_test VALUES ('00000000-0000-7000-8000-000000000000');
UPDATE uuid_guc_test SET id = '00000000-0001-4000-8000-000000000000'
WHERE id = '00000000-0000-7000-8000-000000000000';
-- GUC on (default): UUID v7 validation error
\set ON_ERROR_STOP 0
UPDATE uuid_guc_test SET id = 'ffff0000-0000-4000-8000-000000000000'
WHERE id = '00000000-0001-4000-8000-000000000000';
\set ON_ERROR_STOP 1
ROLLBACK;
-- GUC off: check constraint violation
BEGIN;
INSERT INTO uuid_guc_test VALUES ('00000000-0000-7000-8000-000000000000');
UPDATE uuid_guc_test SET id = '00000000-0001-4000-8000-000000000000'
WHERE id = '00000000-0000-7000-8000-000000000000';
SET timescaledb.enable_hypertable_expansion_for_dml TO off;
\set ON_ERROR_STOP 0
UPDATE uuid_guc_test SET id = 'ffff0000-0000-4000-8000-000000000000'
WHERE id = '00000000-0001-4000-8000-000000000000';
\set ON_ERROR_STOP 1
ROLLBACK;
RESET timescaledb.enable_hypertable_expansion_for_dml;
DROP TABLE uuid_guc_test;
