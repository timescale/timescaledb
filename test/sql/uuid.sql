...
SELECT to_timestamp(id), device, temp
...
SELECT :'chunk_range_start',  to_uuidv7_zeroed(:'chunk_range_start');

-- Exclude all but one chunk
EXPLAIN (verbose, buffers off, costs off, timing off)
SELECT to_timestamp(id), device, temp
FROM uuid_events WHERE id < to_uuidv7_zeroed(:'chunk_range_start');
...
EXPLAIN (verbose, buffers off, costs off, timing off)
SELECT to_timestamp(id), device, temp
FROM uuid_events WHERE id < to_uuidv7_zeroed(:'chunk_range_end')
ORDER BY id DESC;
