#!/usr/bin/env bash
#
# CAgg Performance Simulation Script
#
# Replicates a customer schema at scale (~1TB+), creates hierarchical
# continuous aggregates, and simulates live ingestion with backfill
# to investigate CAgg refresh performance.
#
# Customer reference metrics (for full-scale reproduction):
#   - Distinct devices:    ~7.3M
#   - Distinct sensors:    ~22.5M
#   - Sensors/device:      avg 3.04, min 1, max 37
#   - Rows/chunk (5hr):    ~430M
#   - Rows/day:            ~2B
#   - Rows/sensor/hour:    ~3.9
#   - Total chunks:        ~3028
#   - Size before comp:    ~6.4 TB
#   - Size after comp:     ~1.3 TB
#   - Data span:           ~1 year
#
# Usage:
#   ./cagg_perf_sim.sh --all          # Run all phases
#   ./cagg_perf_sim.sh --setup        # Phase 1: Create schema (no indexes)
#   ./cagg_perf_sim.sh --generate     # Phase 2: Parallel data gen + indexes + CAggs
#   ./cagg_perf_sim.sh --caggs        # Phase 3: Create CAggs (standalone)
#   ./cagg_perf_sim.sh --simulate     # Phase 4: Traffic simulation
#
set -euo pipefail
...
INGEST_RATE="${INGEST_RATE:-100000}"
INGEST_BATCH="${INGEST_BATCH:-10000}"
BACKFILL_RATIO="${BACKFILL_RATIO:-0.13}"
BACKFILL_MAX_DAYS="${BACKFILL_MAX_DAYS:-40}"
...
###############################################################################
# Phase 1: Setup — table, hypertable, sensor pool (NO indexes yet)
###############################################################################
phase_setup() {
	info "=== Phase 1: Setup ==="
	...
	info "Indexes will be created AFTER data generation for speed."
...
###############################################################################
# Phase 2: Parallel data generation + index creation + CAgg creation
#
# Two-pass approach:
#   Pass 1 (Seed):  For EVERY chunk, insert 1 row per sensor so each chunk
#                   has exactly NUM_SENSORS distinct sensor IDs.
#   Pass 2 (Fill):  Random rows to reach TARGET_SIZE_GB (if needed).
###############################################################################

# Seed worker — handles a range of chunks [chunk_start_idx, chunk_end_idx).
# For each chunk, inserts sensor_count sensors (IDs 1..sensor_count) in
# batches with random timestamps within that chunk's time range.
_seed_worker() {
	local worker_id=$1
	local chunk_start_idx=$2
	local chunk_end_idx=$3
	local chunk_interval_secs=$4
	local sensor_count=$5   # sensors_per_chunk (clamped to pool_count)
	local progress_file=$6

	local rows_done=0

	for ((c=chunk_start_idx; c<chunk_end_idx; c++)); do
		# Chunk covers [now - (c+1)*interval, now - c*interval)
		local offset_start=$((c * chunk_interval_secs))
		local chunk_span=$chunk_interval_secs

		# Insert sensors into this chunk, in batches
		local current_id=1
...
# Fill worker — inserts random rows across a time range to reach target size.
_fill_worker() {
	local worker_id=$1
	local day_start=$2
	local day_end=$3
...
# Generic monitor loop — polls progress files until all workers finish.
# Args: label total_rows start_ts progress_dir worker_pids_str(space-separated)
_monitor_workers() {
	local label=$1
	local total_rows=$2
	local start_ts=$3
	...
	while ! $all_done; do
		...
		# Query table size every 30s (every 3rd iteration) to avoid overhead during bulk load
		if [ $((iter % 3)) -eq 0 ]; then
...
phase_generate() {
	...
	# Convert CHUNK_INTERVAL to seconds via PostgreSQL
	...
	# Clamp SENSORS_PER_CHUNK to pool_count
	...
	# ---- Pass 1: Seed — sensors_per_chunk sensors in every chunk ----
	if [ "$SKIP_SEED" != "true" ]; then
	...
	# ---- Pass 2: Fill — random rows to reach target size ----
...
###############################################################################
phase_caggs() {
	info "=== Phase 3: Create Continuous Aggregates ==="

	info "Creating hourly CAgg (readings_hourly)..."
	run_sql <<-'SQL'
		CREATE MATERIALIZED VIEW IF NOT EXISTS hourly_sensor_without_join
		...
		SELECT time_bucket('1 hour', timestamp) AS bucket,
		       ...
		       avg(avg_value) AS agg_avg_value,
		       min(min_value) AS agg_min_value,
		       max(max_value) AS agg_max_value,
		       count(*) AS reading_count
		FROM readings
		GROUP BY bucket, sensor_uuid
		WITH NO DATA;
	SQL
	info "readings_hourly created."

	info "Creating daily CAgg (readings_daily) on top of readings_hourly..."
	run_sql <<-'SQL'
		CREATE MATERIALIZED VIEW IF NOT EXISTS daily_sensor_without_join
		WITH (timescaledb.continuous) AS
		SELECT time_bucket('1 day', bucket) AS bucket,
		       ...
		       avg(agg_avg_value) AS agg_avg_value,
		       min(agg_min_value) AS agg_min_value,
		       max(agg_max_value) AS agg_max_value,
		       sum(reading_count) AS reading_count
		FROM hourly_sensor_without_join
		GROUP BY (time_bucket('1 day'::interval, bucket)), sensor_uuid
		WITH NO DATA;
	SQL
	info "readings_daily created (hierarchical on readings_hourly)."

	info "Adding refresh policies..."
	run_sql <<-'SQL'
		SELECT add_continuous_aggregate_policy('hourly_sensor_without_join',
			start_offset    => INTERVAL '2 hours',
			end_offset    => NULL,
			schedule_interval => INTERVAL '1 hour');

		SELECT add_continuous_aggregate_policy('daily_sensor_without_join',
			start_offset    => INTERVAL '2 days',
			end_offset    => NULL,
			schedule_interval => INTERVAL '1 hour');
	SQL
	info "Refresh policies added."

	info "Phase 3 complete. CAgg status:"
	run_sql -c "SELECT view_name, materialization_hypertable_schema,
...
# Single simulation worker — runs as a background subprocess
_simulate_worker() {
	local worker_id=$1
	local pool_count=$2
	local live_count=$3
	local backfill_count=$4
	local sleep_target=$5
	local progress_file=$6
	...
	while true; do
		...
		# Insert live rows (timestamps in last 5 hours)
		if [ "$live_count" -gt 0 ]; then
			psql "$CONNSTR" -X -q --set ON_ERROR_STOP=off <<-SQL
				INSERT INTO readings (timestamp, sensor_uuid, device_uuid, channel,
				                      value, min_value, max_value, avg_value, sample_count)
				SELECT
					now() - (random() * 5) * INTERVAL '1 hour',
					sp.sensor_uuid,
					sp.device_uuid,
					floor(random() * ${NUM_CHANNELS})::int + 1,
					random() * 1000,
					...
					random() * 1000,
					(random() * 100)::int + 1
				FROM generate_series(1, ${live_count}) AS g(i)
				JOIN _sim_sensor_pool sp ON sp.id = ((g.i - 1) % ${pool_count}) + 1
				;
			SQL
		fi

		# Insert backfill rows (timestamps in the past)
		if [ "$backfill_count" -gt 0 ]; then
			psql "$CONNSTR" -X -q --set ON_ERROR_STOP=off <<-SQL
				INSERT INTO readings (timestamp, sensor_uuid, device_uuid, channel,
				                      value, min_value, max_value, avg_value, sample_count)
				SELECT
					now() - (random() * ${BACKFILL_MAX_DAYS}) * INTERVAL '1 day'
					       - (random() * 86400) * INTERVAL '1 second',
					sp.sensor_uuid,
					sp.device_uuid,
					floor(random() * ${NUM_CHANNELS})::int + 1,
					...
					random() * 1000,
					(random() * 100)::int + 1
				FROM generate_series(1, ${backfill_count}) AS g(i)
				JOIN _sim_sensor_pool sp ON sp.id = ((g.i - 1) % ${pool_count}) + 1
				;
			SQL
		fi

		rows_done=$((rows_done + live_count + backfill_count))
		echo "$rows_done" > "$progress_file"

		local batch_end
		batch_end=$(date +%s%N)
		local batch_elapsed_ms=$(( (batch_end - batch_start) / 1000000 ))
		local target_ms
		target_ms=$(awk "BEGIN { printf \"%d\", ${sleep_target} * 1000 }")
		local sleep_ms=$((target_ms - batch_elapsed_ms))

		if [ "$sleep_ms" -gt 0 ]; then
			sleep "$(awk "BEGIN { printf \"%.3f\", ${sleep_ms} / 1000.0 }")"
		fi
	done
}

phase_simulate() {
	info "=== Phase 4: Traffic Simulation (${PARALLEL_JOBS} workers) ==="
	info "Target rate: ${INGEST_RATE} rows/sec | Batch: ${INGEST_BATCH} | Backfill: ${BACKFILL_RATIO} (max ${BACKFILL_MAX_DAYS} days)"
	...
	# Split batch size across workers
	local per_worker_batch=$(( INGEST_BATCH / PARALLEL_JOBS ))
	local backfill_count
	backfill_count=$(awk "BEGIN { printf \"%d\", ${per_worker_batch} * ${BACKFILL_RATIO} }")
	local live_count=$((per_worker_batch - backfill_count))
	local sleep_target
	sleep_target=$(awk "BEGIN { printf \"%.4f\", ${per_worker_batch} / (${INGEST_RATE}.0 / ${PARALLEL_JOBS}) }")

	info "Workers: ${PARALLEL_JOBS} | Per worker batch: ${live_count} live + ${backfill_count} backfill = ${per_worker_batch} rows"
	info "Target batch interval per worker: ${sleep_target}s"
	echo ""
	...
	for ((w=0; w<PARALLEL_JOBS; w++)); do
		local pfile="${progress_dir}/worker_${w}"
		echo "0" > "$pfile"
		_simulate_worker "$w" "$pool_count" "$live_count" "$backfill_count" "$sleep_target" "$pfile" &
		worker_pids+=($!)
		info "  Started worker ${w} (PID ${worker_pids[$w]})"
	done
	...
	_cleanup_sim() {
		...
		wait 2>/dev/null
		rm -rf "$progress_dir"
		info "Simulation stopped after $(elapsed_since "$sim_start")s"
		exit 0
	}
	trap _cleanup_sim INT
	...
	while true; do
		...
		local elapsed
		elapsed=$(elapsed_since "$sim_start")
		local rate=0
		if [ "$elapsed" -gt 0 ]; then
			rate=$((total_done / elapsed))
		fi

		stat "[Sim] ${total_done} rows | Rate: ${rate} rows/sec | Workers: ${alive} | Elapsed: ${elapsed}s"

		if [ "$alive" -eq 0 ]; then
			err "All workers died unexpectedly."
			rm -rf "$progress_dir"
			exit 1
		fi
	done
...
usage() {
	cat <<-EOF
		Usage: $0 [--setup|--generate|--caggs|--simulate|--all]

		  --setup      Phase 1: Create table, hypertable, sensor pool (no indexes)
		  --generate   Phase 2: Parallel data gen + create indexes + create CAggs
		  --caggs      Phase 3: Create CAggs only (standalone)
		  --simulate   Phase 4: Simulate live ingestion with backfill
		  --all        Run all phases in order

		Configuration via environment variables:
		  CONNSTR            Connection string (default: postgresql://localhost:5432/postgres)
		  NUM_SENSORS        Sensor count (default: 22000000, real: ~22.5M)
		  NUM_DEVICES        Device count (default: 7000000, real: ~7.3M)
		  ...
		  TARGET_SIZE_GB     Target data size in GB (default: 1000, real: ~6400 uncompressed)
		  DATA_SPAN_DAYS     Days of historical data (default: 365)
		  CHUNK_INTERVAL     Chunk interval (default: '5 hours')
		  BATCH_SIZE         Rows per batch during generation (default: 1000000)
		  PARALLEL_JOBS      Number of parallel workers for generation and simulation (default: 4)
		  INGEST_RATE        Target rows/sec during simulation (default: 100000)
		  INGEST_BATCH       Rows per batch during simulation (default: 10000)
		  BACKFILL_RATIO     Fraction of backfill traffic (default: 0.13)
		  BACKFILL_MAX_DAYS  Max backfill age in days (default: 40)

		Quick test: TARGET_SIZE_GB=1 PARALLEL_JOBS=2 ./cagg_perf_sim.sh --all

		Server tuning (recommended for bulk load):
		  ALTER SYSTEM SET max_wal_size = '10GB';
		  ALTER SYSTEM SET checkpoint_completion_target = 0.9;
		  ALTER SYSTEM SET autovacuum = off;  -- re-enable after load!
		  SELECT pg_reload_conf();
	EOF
}

case "${1:-}" in
	--setup|--generate|--caggs|--simulate|--all) ;;
	*)
		usage
		exit 1
		;;
...
info "Config: INGEST_RATE=${INGEST_RATE}, INGEST_BATCH=${INGEST_BATCH}"
info "Config: BACKFILL_RATIO=${BACKFILL_RATIO}, BACKFILL_MAX_DAYS=${BACKFILL_MAX_DAYS}"
echo ""
