...
BEGIN
    ...
    CREATE TRIGGER trigger_meta_on_create_chunk_replica_node AFTER INSERT OR UPDATE OR DELETE ON chunk_replica_node
    ...
    ON chunk;
    CREATE TRIGGER trigger_meta_on_create_chunk AFTER INSERT OR UPDATE OR DELETE ON chunk
    FOR EACH ROW EXECUTE PROCEDURE _meta.on_create_chunk();

    DROP TRIGGER IF EXISTS trigger_2_meta_create_hypertable
    ON hypertable;
    CREATE TRIGGER trigger_2_meta_create_hypertable AFTER INSERT OR UPDATE OR DELETE ON hypertable
    FOR EACH ROW EXECUTE PROCEDURE _meta.on_create_hypertable();

    DROP TRIGGER IF EXISTS trigger_meta_create_node
    ON node;
    CREATE TRIGGER trigger_meta_create_node BEFORE INSERT OR UPDATE OR DELETE ON node
    FOR EACH ROW EXECUTE PROCEDURE _meta.on_create_node();

    DROP TRIGGER IF EXISTS trigger_meta_sync_node
    ON node;
    CREATE TRIGGER trigger_meta_sync_node AFTER INSERT ON node
    FOR EACH ROW EXECUTE PROCEDURE _meta.sync_node();

    DROP TRIGGER IF EXISTS trigger_meta_create_partition
    ON partition;
    CREATE TRIGGER trigger_meta_create_partition AFTER INSERT OR UPDATE OR DELETE ON partition
    ...
    'chunk_replica_node', 'hypertable_column', 'deleted_hypertable_column', 'meta', 'default_replica_node'] :: NAME [] LOOP
        EXECUTE format(
            $$
                ...
                DROP TRIGGER IF EXISTS trigger_0_meta_sync_update_%1$s ON %1$s;
                DROP TRIGGER IF EXISTS trigger_0_meta_sync_delete_%1$s ON %1$s;
            $$,
            table_name);
        EXECUTE format(
            $$
                CREATE TRIGGER trigger_0_meta_sync_insert_%1$s AFTER INSERT ON %1$s
                FOR EACH ROW EXECUTE PROCEDURE _sysinternal.sync_insert();
                CREATE TRIGGER trigger_0_meta_sync_update_%1$s AFTER UPDATE ON %1$s
                FOR EACH ROW EXECUTE PROCEDURE _sysinternal.sync_update();
                CREATE TRIGGER trigger_0_meta_sync_delete_%1$s AFTER DELETE ON %1$s
    ...
    FOREACH table_name IN ARRAY ARRAY ['hypertable_column', 'hypertable_index', 'hypertable'] :: NAME [] LOOP
        ...
        EXECUTE format(
            $$
                CREATE TRIGGER trigger_0_meta_deleted_%1$s BEFORE DELETE ON %1$s
