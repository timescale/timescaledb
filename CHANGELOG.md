...
* Sparse indexes on compressed hypertables can now be explicitly configured via `ALTER TABLE` rather than relying only on internal heuristics. Users can define indexes on multiple columns, apply both minmax and bloom indexes on the same column, and manage configurations through system views—providing more control over compression performance when defaults may not be optimal.  
...
* UUIDv7 compression and vectorization are now supported. An experimental compression algorithm leverages the timestamp portion for delta-delta compression while storing the random portion separately, and vectorized equality/inequality filters with bulk decompression deliver ~2× faster query performance. The feature is disabled by default (`timescaledb.enable_uuid_compression`) and will be enabled in a future release.  
