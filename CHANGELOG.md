...
* Another performance optimization on the columnstore was added, complementary to `ColumnarIndexScan`. For queries using time as the primary dimension, e.g. `SELECT COUNT(*) FROM events WHERE time > '2025-01-20';`, the `time` column is no longer required to get decompressed, showing increased query performance of 50x faster.

**Sunsetting Announcements**
* This release removes the WAL based invalidation of continuous aggregates. This feature was introduced in [2.22.0](https://github.com/timescale/timescaledb/releases/tag/2.22.0) as tech preview to use logical decoding for building the invalidation logs. The feature was designed for high ingest workloads, reduing the write amplification. With the upcoming stream of improvements to continuous aggregates, this feature was deprioritized and removed.
