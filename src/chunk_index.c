...
static Oid
chunk_index_find_matching(Relation chunk_rel, Relation index)
{
	...
	foreach (lc, indexlist)
	{
		...
		if (ts_indexing_compare(indexoid, RelationGetRelid(index)) &&
				   ...
				   int32 chunk_id, Relation chunkrel, Oid constraint_oid, Oid index_tblspc)
{
	...
	chunk_indexrelid = chunk_index_find_matching(chunkrel, hypertable_idxrel);
