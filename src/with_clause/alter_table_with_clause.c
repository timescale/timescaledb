...
sparse_index_type_with_clause_parse(const char *parse, const WithClauseDefinition *args, int nargs)
{
	...
	return SparseIndexTypeMax;
}

static void
convert_to_json(JsonbParseState *parse_state, SparseIndexConfig *config)
{
	pushJsonbValue(&parse_state, WJB_BEGIN_OBJECT, NULL);
	ts_jsonb_add_str(parse_state,
					 SparseIndexCommonKeys[SparseIndexKeyType],
					 SparseIndexTypeNames[config->base.type]); /* type */

	ts_jsonb_add_str(parse_state,
					 SparseIndexCommonKeys[SparseIndexKeyCol],
					 config->base.col); /* col */
		...
		ereport(ERROR,
				(errcode(ERRCODE_SYNTAX_ERROR),
				 errmsg("sparse index %s's first option must reference a valid "
						"column.",
						SparseIndexTypeNames[config.base.type])));
	...
	*collist = ts_array_add_element_text(*collist, pstrdup(colname));
	...
	switch (config.base.type)
	{
		case SparseIndexTypeBloom:
			...
			//  ?????????
			//  FmgrInfo *finfo = NULL;
			//   if (bloom1_get_hash_function(coltypid, &finfo) == NULL)
			//   	ereport(ERROR,
			//   			(errcode(ERRCODE_UNDEFINED_FUNCTION),
			//   			 errmsg("invalid bloom filter column type %s",
			//   format_type_be(coltypid)), 			 errdetail("Could not identify a hashing
			//   function for the type.")));

			config.base.col = colname;
			config.custom.bloom.dummy = 0;
			break;
		case SparseIndexTypeMinmax:
	...
	convert_to_json(parse_state, &config);
	return;
}
...
parse_sparse_index_config_list(char *inpstr, Hypertable *hypertable)
{
	...
	/* json format will be [{"col": "u", "type": "bloom"}, {"col": "ts", "type": "minmax"}] */
