...
static Const *
internal_timeval_to_const(int64 value, Oid type)
{
...
static int64
const_to_internal_timeval(const Const *cnst)
{
...
extract_time_bucket_qual(Expr *node, TimeBucketQual *tbqual)
{
	...
	else if (IsA(right, FuncExpr) && IsA(left, Const))
	{
		...
		tbqual->opno = get_commutator(op->opno);
	}
	else
	{
		return false;
	...
	tbqual->strategy = get_op_opfamily_strategy(tbqual->opno, tce->btree_opf);
							  ...
							  int64 *integral_width)
{
	switch (width->consttype)
	{
		...
		case INT8OID:
			 ...
			 * width.
			 */
			*integral_width = const_to_internal_timeval(width);
...
ts_transform_time_bucket_comparison(Expr *node)
{
	 ...
	 * to internal time format.
	 */
	int64 integral_value = const_to_internal_timeval(tbqual.value);
	Const *newvalue = NULL;
	...
	switch (tbqual.strategy)
	{
		...
		case BTGreaterEqualStrategyNumber:
			 ...
			 * column > value
			 */
			newvalue = internal_timeval_to_const(integral_value, tbqual.tb.timetype);
			break;
		...
		case BTLessEqualStrategyNumber:
		{
			...
			if (tbqual.strategy == BTLessStrategyNumber && tbqual.tb.numargs == 2 &&
				...
				newvalue =
					internal_timeval_to_const(integral_value + integral_width, tbqual.tb.timetype);
