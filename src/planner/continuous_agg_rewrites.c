...
check_query_for_cagg_rewrites(CaggRewriteContext *cagg_rewrite_ctx, Query *query)
{
	...
	cagg_rewrite_ctx->eligible =
		ts_cagg_query_supported(query, &(cagg_rewrite_ctx->msg), NULL, for_rewrites);
}
...
match_expr(Node *expr1, Node *expr2)
{
	...
	/* Time buckets are matched separately */
		...
		return true;
	/* Check for equivalent OpExprs like "a=5" and "5=a" */
	else if (IsA(expr1, OpExpr) && IsA(expr2, OpExpr))
	{
		...
		if (list_length(op1->args) == 2 && list_length(op2->args) == 2 &&
			match_lists(op1->args, op2->args) && match_lists(op2->args, op1->args))
		{
#if PG18_GE
			List *op1infos = get_op_index_interpretation(op1->opno);
			List *op2infos = get_op_index_interpretation(op2->opno);
#else
			List *op1infos = get_op_btree_interpretation(op1->opno);
			List *op2infos = get_op_btree_interpretation(op2->opno);
#endif
			if (list_length(op1infos) && list_length(op2infos))
			{
#if PG18_GE
				CompareType cmp1 = ((OpIndexInterpretation *) linitial(op1infos))->cmptype;
				CompareType cmp2 = ((OpIndexInterpretation *) linitial(op2infos))->cmptype;
				/* 1(LT)+5(GT) or 2(LE)+4(GE) or 3(EQ)+3(EQ) == 6(NE) */
				if (cmp1 + cmp2 == COMPARE_NE)
					return true;
#else
				int cmp1 = ((OpBtreeInterpretation *) linitial(op1infos))->strategy;
				int cmp2 = ((OpBtreeInterpretation *) linitial(op2infos))->strategy;
				/* 1(LT)+5(GT) or 2(LE)+4(GE) or 3(EQ)+3(EQ) */
				if (cmp1 + cmp2 == BTMaxStrategyNumber + 1)
					return true;
#endif
			}
		}
...
match_query_to_cagg(CaggRewriteContext *cagg_rewrite_ctx, Query *query, bool do_rewrite)
{
	...
	foreach (l, caggs)
	{
		...
		foreach (lsrc, newquery->targetList)
		{
			...
			if (tle->resjunk)
				continue;
			Node *new_entry = replace_tlist_expr((Node *) tle, itlist, 1, 0);
			if (!new_entry)
			{
				new_tlist = NULL;
				break;
