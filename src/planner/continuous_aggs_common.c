							 ...
							 const bool for_rewrites)
{
	if (rte->rtekind == RTE_RELATION)
	{
		...
		if (is_hypertable && rte->inh == false)
		{
			if (!for_rewrites)
				appendStringInfoString(detail,
								 ...
								 StringInfo detail, StringInfo hint, const bool for_rewrites)
{
	...
	if (part_dimension == NULL || part_dimension->partitioning != NULL)
	{
		if (msg)
			appendStringInfoString(msg,
								   "custom partitioning not supported with continuous aggregates");
		return NULL;
	}
							  ...
							  bool is_cagg_create, AttrNumber htpartcolno, StringInfo msg)
{
	...
	if (htpartcolno != InvalidAttrNumber &&
	...
	if (nargs >= 3)
	{
		Const *arg = check_time_bucket_argument(lthird(fe->args), "third", process_checks, msg);
		if (!arg)
			return;
	...
	if (nargs >= 4)
	{
		Const *arg = check_time_bucket_argument(lfourth(fe->args), "fourth", process_checks, msg);
		if (!arg)
			return;
		if (!process_additional_timebucket_parameter(bf, arg, &custom_origin, process_checks))
			return;
	...
	if (nargs == 5)
	{
		Const *arg = check_time_bucket_argument(lfifth(fe->args), "fifth", process_checks, msg);
		if (!arg)
			return;
		if (!process_additional_timebucket_parameter(bf, arg, &custom_origin, process_checks))
			return;
	...
	if (custom_origin && TIMESTAMP_NOT_FINITE(bf->bucket_time_origin))
	{
		if (msg)
						   ...
						   bool for_rewrite)
{
	...
	foreach (l, group_exprs)
	{
		...
		if (IsA(expr, FuncExpr))
		{
			...
			process_timebucket_parameters(fe, bf, true, is_cagg_create, ht_partcolno, msg);
			if (bf->bucket_function == InvalidOid)
				return false;
		}
	}
...
ts_cagg_get_bucket_function_info(Oid view_oid)
{
	...
	foreach (l, query->groupClause)
	{
		...
		if (IsA(expr, FuncExpr))
		{
			...
			process_timebucket_parameters(fe, bf, false, false, InvalidAttrNumber, NULL);
			break;
		}
	}
