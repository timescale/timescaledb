...
struct ext {
	...
	char const *guc_disable_load_name;
...
inline static void extension_check(struct ext *);
#if PG14_LT
static void call_extension_post_parse_analyze_hook(ParseState *pstate, Query *query, struct ext const *);
#else
static void call_extension_post_parse_analyze_hook(ParseState *pstate, Query *query, struct ext const *,
												   JumbleState *jstate);
#endif
...
struct ext extensions[] = {
	{ "timescaledb", "timescaledb.disable_load" },
//	{ "timescaledb_osm", "timescaledb_osm.disable_load" },
};

static bool
extension_is_loaded(struct ext const *const ext)
{
...
static bool
drop_statement_drops_extension(DropStmt const *const stmt, struct ext const *const ext)
{
...
static Oid
extension_owner(struct ext const *const ext)
{
	Datum result;
	Relation rel;
...
static bool
drop_owned_statement_drops_extension(DropOwnedStmt const *const stmt, struct ext const *const ext)
{
...
static bool
should_load_on_variable_set(Node const *const utility_stmt, struct ext const *const ext)
{
...
static bool
should_load_on_alter_extension(Node const *const utility_stmt, struct ext const *const ext)
{
...
static bool
should_load_on_create_extension(Node const *const utility_stmt, struct ext const *const ext)
{
...
static bool
load_utility_cmd(Node const *const utility_stmt, struct ext const *const ext)
{
...
#endif
{
	for (size_t i = 0; i < sizeof extensions / sizeof(struct ext); ++i) {
		struct ext *const ext = &extensions[i];
	if (query->commandType == CMD_UTILITY)
	{
		switch (nodeTag(query->utilityStmt))
		{
			...
			case T_DropStmt:
				if (drop_statement_drops_extension((DropStmt *) query->utilityStmt, ext))
			...
			case T_DropOwnedStmt:
				if (drop_owned_statement_drops_extension((DropOwnedStmt *) query->utilityStmt, ext))
					ts_bgw_message_send_and_wait(RESTART, MyDatabaseId);
				break;
			case T_RenameStmt:
...
do_load(struct ext *const ext)
{
	 ...
	 */
	// NOTA BENE:  Not checking ext->name because the only other extension we
	// load is timescaledb_osm, which has no 0.9.x in the wild.  If we start
	// loading a third extension which DOES, we'll need to check that here.
...
inline static void
extension_check(struct ext *const ext)
{
	switch (extension_current_state(ext->name))
	{
...
{
	// TODO Don't hard-code 0 as timescaledb.
	extension_check(&extensions[0]);
}

static void
#if PG14_LT
call_extension_post_parse_analyze_hook(ParseState *pstate, Query *query, struct ext const *const ext)
#else
call_extension_post_parse_analyze_hook(ParseState *pstate, Query *query, struct ext const *const ext, JumbleState *jstate)
#endif
{
