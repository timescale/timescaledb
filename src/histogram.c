...
copy_state(MemoryContext aggcontext, bytea *state) {
	...
	Datum *hist, *hist_copy;

	copy = MemoryContextAllocZero(aggcontext, VARHDRSZ + arrsize);
	SET_VARSIZE(copy, VARHDRSZ + arrsize);

	hist = (Datum *) VARDATA(state);
	hist_copy = (Datum *) VARDATA(copy);

	for (i = 0; i < (arrsize / sizeof(Datum)); i++)
		hist_copy[i] = hist[i];

	PG_RETURN_BYTEA_P(copy);
}
...
hist_combinefunc(PG_FUNCTION_ARGS)
{
	...
	if (state2 == NULL) {
		result = (bytea *) copy_state(aggcontext, state1);
	}

	else if (state1 == NULL) {
		result = (bytea *) copy_state(aggcontext, state2);
	}
