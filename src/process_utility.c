...
process_alter_column_type_start(ParseState *pstate, Hypertable *ht, AlterTableCmd *cmd)
{
	...
	if (is_granular_refresh_tracking_column(ht, cmd->name))
	{
		ereport(ERROR,
				(errcode(ERRCODE_TS_OPERATION_NOT_SUPPORTED),
				 errmsg("cannot change type of column \"%s\" used to set up granular refresh",
						cmd->name),
				 errdetail("Granular refresh on hypertable \"%s\" tracks changes using this "
						   "column.",
						   get_rel_name(ht->main_table_relid))));
...
process_altertable_set_options(AlterTableCmd *cmd, Hypertable *ht)
{
	...
	if (!parse_results[AlterTableFlagColumnstore].is_default ||
		...
		!parse_results[AlterTableFlagConcurrentCompress].is_default ||
		!parse_results[AlterTableFlagConcurrentCompressScheduleInterval].is_default)
	{
	...
	if (ht && (!parse_results[AlterTableFlagGranularRefreshColumn].is_default ||
			   ...
			   !parse_results[AlterTableFlagCaggEnableGranularRefresh].is_default))
	{
