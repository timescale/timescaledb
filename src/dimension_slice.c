...
ts_correlated_constraints_dimension_slice_insert(const Hypertable *ht, Chunk *chunk)
{
	...
	for (i = 0; i < hs->num_dimensions; i++)
	{
		...
		ts_chunk_constraints_add(chunk->constraints, chunk->fd.id, slice->fd.id, NULL, NULL, true);
	}
														   ...
														   bool reset)
{
		 ...
		 constraint_index++)
	{
		...
		if (!is_correlated_constraint_dimension(constraint))
			continue;
		...
		if (ti == NULL)
			elog(ERROR, "dimension slice %d not found!", slice_id);
		...
		if (dim == NULL)
			elog(ERROR, "correlated constraint dimension %d not found!", slice->fd.dimension_id);

		attno = ts_map_attno(ht->main_table_relid, chunk->table_id, dim->column_attno);

		if (reset)
		{
			minmax[0] = Int64GetDatum(DIMENSION_SLICE_MINVALUE);
			minmax[1] = Int64GetDatum(DIMENSION_SLICE_MAXVALUE);
		}
										 ...
										 minmax))
		{
			...
			DimensionVec *vec;
			int64 min = DatumGetInt64(minmax[0]);
			int64 max = DatumGetInt64(minmax[1]);
			...
			elog(WARNING, "Not able to calculate min/max values for correlated constraints");
