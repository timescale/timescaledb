...
ts_chunk_get_osm_slice_and_lock(int32 osm_chunk_id, int32 time_dim_id)
{
	...
	ScanIterator slice_it = ts_dimension_slice_scan_iterator_create(NULL, CurrentMemoryContext);
	...
	for (int i = 0; i < constraints->num_constraints; i++)
	{
		...
		if (is_dimension_constraint(cc))
		{
			...
			ScanTupLock *const tuplock_ptr = &tuplock;

			dimslice = ts_dimension_slice_scan_iterator_get_by_id(&slice_it,
																  cc->fd.dimension_slice_id,
																  tuplock_ptr);
			Assert(dimslice != NULL);
			if (dimslice->fd.dimension_id == time_dim_id)
				slice = dimslice;
		}
	}
	ts_scan_iterator_close(&slice_it);
	return slice;
}
/*
