...
TSDLLEXPORT const char *ts_sparse_index_common_keys[] = { "type", "column", NULL };
			...
			else if (settings->fd.index)
			{
				for (int i = 0; i < _SparseIndexTypeEnumMax; i++)
				{
														...
														ts_sparse_index_type_names[i]))
					{
						ereport(ERROR,
								(errcode(ERRCODE_SYNTAX_ERROR),
								 errmsg("the orderby column \"%s\" can only have an implicit "
										"minmax sparse index",
										TextDatumGetCString(datum)),
								 errhint(
									 "Use separate columns for the timescaledb.compress_orderby and"
									 " timescaledb.compress_index options.")));
					}
				}
			}
		}
					...
					ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("the segmentby column \"%s\" can not have sparse "
									"indexes",
									TextDatumGetCString(datum)),
							 errhint("Use separate columns for the timescaledb.compress_segment and"
									 " timescaledb.compress_index options.")));
				}
			}
		}
