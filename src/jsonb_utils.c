...
ts_jsonb_equal(Jsonb *left, Jsonb *right)
{
	...
	Datum result = OidFunctionCall2(F_JSONB_EQ, PointerGetDatum(left), PointerGetDatum(right));

	return DatumGetBool(result);
}

/*
 * Searches for any occurrences of a matching key value pair (strings only).
