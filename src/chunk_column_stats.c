...
enum Anum_enable_chunk_column_stats
{
	...
	Anum_enable_chunk_column_stats_created,
...
enum Anum_disable_chunk_column_stats
{
	...
	Anum_disable_chunk_column_stats_dropped,
 ...
 */
static Datum
chunk_column_stats_create_datum(FunctionCallInfo fcinfo, Form_chunk_column_stats info, bool created)
{
	TupleDesc tupdesc;
	HeapTuple tuple;
	...
	values[AttrNumberGetAttrOffset(Anum_enable_chunk_column_stats_id)] = Int32GetDatum(info->id);
	values[AttrNumberGetAttrOffset(Anum_enable_chunk_column_stats_created)] = BoolGetDatum(created);
 ...
 */
static Datum
chunk_column_stats_remove_datum(FunctionCallInfo fcinfo, int32 hypertable_id, Name colname,
								bool dropped)
{
	TupleDesc tupdesc;
	HeapTuple tuple;
	...
	values[AttrNumberGetAttrOffset(Anum_disable_chunk_column_stats_hypertable_id)] = hypertable_id;
	values[AttrNumberGetAttrOffset(Anum_disable_chunk_column_stats_column_name)] =
		NameGetDatum(colname);
	values[AttrNumberGetAttrOffset(Anum_disable_chunk_column_stats_dropped)] =
		BoolGetDatum(dropped);
	tuple = heap_form_tuple(tupdesc, values, nulls);
...
chunk_column_stats_insert_relation(const Relation rel, Form_chunk_column_stats info)
{
	...
	values[AttrNumberGetAttrOffset(Anum_chunk_column_stats_column_type)] =
		ObjectIdGetDatum(info->column_type);
	...
	values[AttrNumberGetAttrOffset(Anum_chunk_column_stats_valid)] = Int64GetDatum(info->valid);
...
chunk_column_stats_insert(Form_chunk_column_stats info)
{
	...
	dim_range_id = chunk_column_stats_insert_relation(rel, info);
							   ...
							   bool if_not_exists)
{
	...
	info->column_type = DatumGetObjectId(datum);
			...
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("data type \"%s\" unsupported for range calculation",
							format_type_be(info->column_type)),
 ...
 */
static Datum
ts_chunk_column_stats_add_internal(FunctionCallInfo fcinfo, Oid table_relid, NameData colname,
								   bool if_not_exists)
{
	Hypertable *ht;
	Cache *hcache;
	Datum retval = 0;
	int32 dim_range_id = 0;
		...
		if (!if_not_exists)
		{
			ereport(ERROR,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("dimension range entry already exists for column \"%s\"",
							NameStr(colname))));
		}
		else
		{
			ereport(NOTICE,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("dimension range entry already exists for column \"%s\", skipping",
							NameStr(colname))));
			/* return the existing id */
			dim_range_id = form->id;
			created = false;
			goto do_return;
		}
	}
	...
	fd.valid = true;
	dim_range_id = chunk_column_stats_insert(&fd);

	/*
	 * If the hypertable has chunks, to make it compatible
	 * we add artificial dimension range entries which will cover -inf / inf
...
do_return:
	...
	fd.id = dim_range_id;
	retval = chunk_column_stats_create_datum(fcinfo, &fd, created);
...
ts_chunk_column_stats_disable(PG_FUNCTION_ARGS)
{
	...
	if (ts_chunk_column_stats_lookup(ht->fd.id, INVALID_CHUNK_ID, NameStr(colname)) == NULL)
	{
		if (!if_not_exists)
		{
			ereport(ERROR,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("dimension range entry does not exist for column \"%s\"",
							NameStr(colname))));
		}
		else
		{
			ereport(NOTICE,
					(errcode(ERRCODE_DUPLICATE_OBJECT),
					 errmsg("dimension range entry does not exist for column \"%s\", skipping",
							NameStr(colname))));
			goto do_return;
		}
	}
...
do_return:
	retval = chunk_column_stats_remove_datum(fcinfo, ht->fd.id, &colname, delete_count > 0);
	ts_cache_release(hcache);
...
ts_chunk_column_stats_fill_dummy_dimension(FormData_chunk_column_stats *r, Oid main_table_relid)
{
	...
	d->fd.aligned = true;
	d->fd.column_type = r->column_type;
...
ts_chunk_column_stats_calculate(const Hypertable *ht, const Chunk *chunk)
{
	...
	for (int range_index = 0; range_index < rs->num_range_cols; range_index++)
	{
		...
		if (ts_chunk_get_minmax(chunk->table_id, col_type, attno, "column dimension range", minmax))
		{
			...
			ereport(WARNING,
					errmsg("unable to calculate min/max values for column dimension ranges"));
	}
...
/*
 * Check if there is a dimension range tracking on this column which is being dropped.
...
ts_chunk_column_stats_drop(const Hypertable *ht, const char *col_name, bool *dropped)
{
	...
	*dropped = false;
	...
	delete_count = ts_chunk_column_stats_delete_by_ht_colname(ht->fd.id, col_name);
	if (delete_count > 0)
		*dropped = true;
}
...
/*
 * For dimension ranges we are interested in the occurrence of a value which
