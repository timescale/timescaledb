...
static MemoryContext PartChunkCacheCtx = NULL;
...
ts_partition_cache_insert_chunk(const Hypertable *ht, Oid chunk_relid)
{
	...
	{
		if (PartChunkCacheCtx == NULL)
			PartChunkCacheCtx = AllocSetContextCreate(CacheMemoryContext,
	...
	MemoryContext oldctx = MemoryContextSwitchTo(PartChunkCacheCtx);
	entry = hash_search(PartChunkCache, &ht->main_table_relid, HASH_ENTER, &found);
	if (!found)
	{
		entry->ht = (Hypertable *) ht;
...
ts_partition_cache_destroy(void)
{
	...
	if (PartChunkCacheCtx != NULL)
	{
		MemoryContextDelete(PartChunkCacheCtx);
		PartChunkCacheCtx = NULL;
	}
}
		...
		if (attribute->atthasdef)
		{
			...
			if (this_default == NULL)
				elog(ERROR, "default expression not found for attribute %d of relation \"%s\"",
						parent_attno, RelationGetRelationName(rel));
 ...
 */
void
ts_partition_chunk_attach(const Hypertable *ht, const Chunk *chunk)
{
	const Dimension *dim = ts_hyperspace_get_dimension_by_id(ht->space,
