 ...
 */
static void
relation_set_reloption_impl(Relation rel, List *defList, LOCKMODE lockmode)
{
	...
	Datum newOptions =
		transformRelOptions(isnull ? (Datum) 0 : datum, defList, NULL, NULL, false, false);
 ...
 */
void
ts_relation_set_reloption(Relation rel, List *defList, LOCKMODE lockmode)
{
	relation_set_reloption_impl(rel, defList, lockmode);
	if (OidIsValid(rel->rd_rel->reltoastrelid))
	{
		Relation toastrel = table_open(rel->rd_rel->reltoastrelid, lockmode);
		relation_set_reloption_impl(toastrel, defList, lockmode);
