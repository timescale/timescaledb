...
contain_mutable_functions_checker(Oid func_id, void *context)
{
	...
	if (ts_pushdown_functions == PUSHDOWN_FUNCTIONS_STABLE)
	{
		/* Push down all stable functions. */
		return func_volatile(func_id) == PROVOLATILE_VOLATILE;
	}

	/* Only push down the stable functions that are known to safe to push down
	 * to the data node. */
