...
static bool
is_vector_var(VectorAggPlanInfo *vpinfo, const Var *var, bool *is_segmentby)
{
	const CustomScan *custom = castNode(CustomScan, vpinfo->childplan);
	...
	for (; compressed_column_index < list_length(decompression_map); compressed_column_index++)
	{
		...
		if (uncompressed_chunk_attno == var->varattno)
		{
			break;
		}
	}
	Ensure(compressed_column_index < list_length(decompression_map), "compressed column not found");
	return is_vector_compressed_column(custom, compressed_column_index, is_segmentby);
}

static VectorQualInfo
build_aggfilter(struct VectorAggPlanInfo *vpinfo)
{
	const CustomScan *custom = castNode(CustomScan, vpinfo->childplan);
...
extern bool
vectoragg_plan_info_init_decompress_chunk(struct VectorAggPlanInfo *vpinfo, Relation rel)
{
	vpinfo->is_vector_var = is_vector_var;
	vpinfo->build_aggfilter = build_aggfilter;

	if (vpinfo->childplan->plan.qual != NIL)
	{
