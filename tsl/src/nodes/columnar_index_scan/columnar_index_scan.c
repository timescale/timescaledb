...
/*
 * first and last take their order from the second argument. Check that the
 * type of the call orders values the same way as the column the metadata was
 * built from, a cast of the order argument can ask for a different order.
 */
static bool
first_last_order_matches_column(Aggref *aggref, Var *var)
{
	...
	return OidIsValid(order_tce->lt_opr) && order_tce->lt_opr == column_tce->lt_opr;
}
