					   ...
					   AttrNumber *metadata_attno)
{
	...
	switch (aggref->aggfnoid)
	{
		...
		case F_MIN_TIMESTAMPTZ:
			...
			*metadata_attno = compressed_column_metadata_attno(info->settings,
							...
							AttrNumber *metadata_attno)
{
	...
	foreach (lc, parse->targetList)
	{
		...
		else
		{
			...
			Node *expr = strip_implicit_coercions((Node *) te->expr);
			if (!IsA(expr, Var))
				return false;
...
cost_columnar_index_scan(ColumnarIndexScanPath *path, Path *compressed_path)
{
	...
	/*
	 * Set low cost to prefer this path over full decompression.
	 * Cost is proportional to number of compressed rows.
	 */
	path->custom_path.path.startup_cost = compressed_path->startup_cost;
	path->custom_path.path.total_cost = compressed_path->total_cost * 0.2;
								...
								List *output_targetlist, List *clauses, List *custom_plans)
{
	...
	{
		TargetEntry *te = (TargetEntry *) lfirst(lc);
		Ensure(IsA(te->expr, Var), "output targetlist entries must be Vars");
