...
{
	return TupIsNull(compressed_batch_get_current_tuple(batch_array_get_at(&bq->batch_array, 0)));
}
...
batch_queue_fifo_pop(BatchQueue *bq, DecompressContext *dcontext)
{
	...
	if (TupIsNull(compressed_batch_get_current_tuple(batch_state)))
	{
							...
							TupleTableSlot *compressed_slot)
{
	...
	Assert(TupIsNull(compressed_batch_get_current_tuple(batch_array_get_at(batch_array, 0))));
...
{
	return compressed_batch_get_current_tuple(batch_array_get_at(&bq->batch_array, 0));
}
