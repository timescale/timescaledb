...
inline static void batch_queue_fifo_create(DecompressChunkState *chunk_state)
{
	batch_array_create(chunk_state, 1);
}

inline static void batch_queue_fifo_free(DecompressChunkState *chunk_state)
{
	batch_array_destroy(chunk_state);
}

inline static bool batch_queue_fifo_needs_next_batch(DecompressChunkState *chunk_state)
{
	return TupIsNull(batch_array_get_at(chunk_state, 0)->decompressed_scan_slot);
}


pg_attribute_always_inline static void batch_queue_fifo_pop(DecompressChunkState *chunk_state)
{
...
inline static void batch_queue_fifo_push_batch(DecompressChunkState *chunk_state, TupleTableSlot *compressed_slot)
{
	DecompressBatchState *batch_state = batch_array_get_at(chunk_state, 0);
...
inline static void batch_queue_fifo_reset(DecompressChunkState *chunk_state)
{
	batch_array_free_at(chunk_state, 0);
}

inline static TupleTableSlot *batch_queue_fifo_top_tuple(DecompressChunkState *chunk_state)
{
