...
/* Check if the provided plan is a ColumnarScan plan */
bool
							 ...
							 List *output_targetlist, List *clauses, List *custom_plans)
{
	...
	settings = ts_new_list(T_IntList, CSS_Count);
	lfirst_int(list_nth_cell(settings, CSS_HypertableId)) = dcpath->info->hypertable_id;
	lfirst_int(list_nth_cell(settings, CSS_ChunkRelid)) = dcpath->info->chunk_rte->relid;
	lfirst_int(list_nth_cell(settings, CSS_Reverse)) = dcpath->reverse;
	lfirst_int(list_nth_cell(settings, CSS_BatchSortedMerge)) = dcpath->batch_sorted_merge;
	lfirst_int(list_nth_cell(settings, CSS_EnableBulkDecompression)) = enable_bulk_decompression;
	lfirst_int(list_nth_cell(settings, CSS_HasRowMarks)) = root->parse->rowMarks != NIL;
	...
	decompress_plan->custom_private = ts_new_list(T_List, CSP_Count);
	lfirst(list_nth_cell(decompress_plan->custom_private, CSP_Settings)) = settings;
	lfirst(list_nth_cell(decompress_plan->custom_private, CSP_DecompressionMap)) =
		context.decompression_map;
	lfirst(list_nth_cell(decompress_plan->custom_private, CSP_IsSegmentbyColumn)) =
		context.is_segmentby_column;
	lfirst(list_nth_cell(decompress_plan->custom_private, CSP_BulkDecompressionColumn)) =
		context.bulk_decompression_column;
	lfirst(list_nth_cell(decompress_plan->custom_private, CSP_SortInfo)) = sort_options;
