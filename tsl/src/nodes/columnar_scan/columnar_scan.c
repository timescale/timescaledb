...
vector_qual_state_get_arrow_array(VectorQualState *vqstate, const Var *var, bool *is_default_value)
{
...
static bool
columnar_scan_is_vector_column(const VectorQualInfo *vqinfo, AttrNumber attno)
{
	const VectorQualInfoHyperstore *vqih = (const VectorQualInfoHyperstore *) vqinfo;
	const ColumnCompressionSettings *column =
		&vqih->hsinfo->columns[AttrNumberGetAttrOffset(attno)];

	Assert(column->attnum == attno);

	return (!column->is_segmentby && column->attnum != InvalidAttrNumber &&
						  ...
						  List *scan_clauses, List *custom_plans)
{
	...
	VectorQualInfoHyperstore vqih = {
		.vqinfo = {
			.rti = rel->relid,
			.is_vector_column = columnar_scan_is_vector_column,
		},
