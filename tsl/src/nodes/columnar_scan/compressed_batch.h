...
typedef struct DecompressBatchState
{
	...
	/*
	 * The shared batch service owns the decoded columns. total_batch_rows is
	 * a snapshot of the payload's validated count (kept after discard for
	 * instrumentation), per_batch_context is a read-only alias of the
	 * payload's value context (never reset or deleted through this struct),
	 * and compressed_columns aliases the payload's stable wrapper array.
	 */
	...
	/*
	 * Alias of the payload's wrapper array. Follows
	 * DecompressContext.compressed_chunk_columns (leading data columns only).
	 */
...
/*
 * Initialize the batch memory context. Bulk decompression scratch context
 * creation moved to compression/column_values.h.
 *
