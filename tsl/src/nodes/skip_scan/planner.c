...
typedef struct SkipScanPath
{
	...
	/* attribute number of the column on the index which provides comparison column
	 * index_attno = distinct_attno if index relation is the same as the table/chunk above
	 * it can be different if distinct value is on decompressed chunk over compressed index
	 */
...
setup_index_plan(CustomScan *skip_plan, Plan *child_plan)
{
	...
	else if (ts_is_decompress_chunk_plan(child_plan))
	{
		...
		/* Set up DecompressChunk plan for "unique segmentby" processing */
		List *settings = lfirst(list_nth_cell(csplan->custom_private, DCP_Settings));
		lfirst_int(list_nth_cell(settings, DCS_UniqueSegBy)) = true;
		/* return compressed index plan */
				 ...
				 SkipScanPath *skip_scan_path)
{
	...
	RelOptInfo *index_rel = index_path->path.parent;
	...
	{
		skip_scan_path->index_attno = var->varattno;
		return var;
	}

	RangeTblEntry *ht_rte = planner_rt_fetch(var->varno, root);
	RangeTblEntry *chunk_rte = planner_rt_fetch(rel->relid, root);
	RangeTblEntry *index_rte =
		(index_rel == rel ? chunk_rte : planner_rt_fetch(index_rel->relid, root));
	...
	if (ts_is_decompress_chunk_path(child_path))
	{
		...
		Relation idx_rel = table_open(index_rte->relid, AccessShareLock);
		TupleDesc idx_desc = RelationGetDescr(idx_rel);
		table_close(idx_rel, NoLock);

		int i;
		for (i = 0; i < idx_desc->natts; i++)
		{
			Form_pg_attribute inatt = TupleDescAttr(idx_desc, i);
		...
		skip_scan_path->index_attno = i + 1;
	}
...
build_skip_qual(PlannerInfo *root, SkipScanPath *skip_scan_path, IndexPath *index_path, Var *var)
{
	 ...
	 */
	int idx_key = get_idx_key(index_path->indexinfo, skip_scan_path->index_attno);
	if (idx_key < 0)
		return NULL;
	...
	Expr *current_val = (Expr *) makeVar(info->rel->relid /*varno*/,
										 skip_scan_path->index_attno /*varattno*/,
										 column_type /*vartype*/,
										 -1 /*vartypmod*/,
