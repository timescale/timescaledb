...
#include "move_hyper_inval_api.h"
...
Datum
policy_move_hyper_inval_proc(PG_FUNCTION_ARGS)
{
	...
	policy_move_hyper_inval_execute(PG_GETARG_INT32(0), PG_GETARG_JSONB_P(1));

	PG_RETURN_VOID();
}

Datum
policy_move_hyper_inval_check(PG_FUNCTION_ARGS)
{
	if (PG_ARGISNULL(0))
	{
		ereport(ERROR,
	...
	policy_move_hyper_inval_read_and_validate_config(PG_GETARG_JSONB_P(0), NULL);

	PG_RETURN_VOID();
}

static int32
policy_move_hyper_inval_add_internal(Hypertable *ht, bool if_not_exists,
									 Interval *schedule_interval, Oid owner_id,
									 TimestampTz initial_start, const bool fixed_schedule,
									 char *timezone, const NullableDatum *include_tiered_data)
{
	NameData application_name, proc_name, proc_schema, check_schema, check_name, owner;

	List *jobs = ts_bgw_job_find_by_proc_and_hypertable_id(POLICY_MOVE_HYPER_INVAL_PROC_NAME,
	...
	namestrcpy(&proc_name, POLICY_MOVE_HYPER_INVAL_PROC_NAME);
	namestrcpy(&proc_schema, FUNCTIONS_SCHEMA_NAME);
	namestrcpy(&check_name, POLICY_MOVE_HYPER_INVAL_CHECK_NAME);
	...
	if (!include_tiered_data->isnull)
		ts_jsonb_add_bool(parse_state,
						  POLICY_CONFIG_KEY_INCLUDE_TIERED_DATA,
...
Datum
policy_move_hyper_inval_add(PG_FUNCTION_ARGS)
{
	...
	text *const timezone = PG_ARGISNULL(4) ? NULL : PG_GETARG_TEXT_PP(4);
	const NullableDatum include_tiered_data = {
		.value = PG_GETARG_DATUM(5),
		.isnull = PG_ARGISNULL(5),
	};
	...
	if (list_length(caggs) == 0)
		ereport(ERROR,
				...
				errmsg("\"%s\" does not have a continuous aggregate attached",
					   get_rel_name(ht_oid)));

	Oid owner_id = ts_hypertable_permissions_check(ht_oid, GetUserId());
	ts_bgw_job_validate_job_owner(owner_id);

	int32 job_id = policy_move_hyper_inval_add_internal(ht,
														if_not_exists,
														schedule_interval,
														owner_id,
														initial_start,
...
static void
policy_move_hyper_inval_remove_internal(Oid ht_oid, bool if_exists)
{
	Cache *hcache;
	Hypertable *ht = ts_hypertable_cache_get_cache_and_entry(ht_oid, CACHE_FLAG_NONE, &hcache);
	if (!ht)
...
Datum
policy_move_hyper_inval_remove(PG_FUNCTION_ARGS)
{
	ts_feature_flag_check(FEATURE_POLICY);
	policy_move_hyper_inval_remove_internal(PG_GETARG_OID(0), PG_GETARG_BOOL(1));
	PG_RETURN_VOID();
}
