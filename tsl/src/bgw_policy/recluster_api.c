...
/* Default max runtime for a recluster job is unlimited for now */
...
/* Right now, there is an infinite number of retries for recluster jobs */
#define DEFAULT_MAX_RETRIES	-1
/* Default retry period for recluster_jobs is currently 1 day */
	...
	if (!HeapTupleIsValid(idxtuple))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("could not add recluster policy because the provided index is not a valid relation")));

	indexForm = (Form_pg_index) GETSTRUCT(idxtuple);
	if (indexForm->indrelid != ht->main_table_relid)
		elog(ERROR, "could not add recluster policy because the provided index is not a valid index on the hypertable");
	ReleaseSysCache(idxtuple);
}

Datum
recluster_add_policy(PG_FUNCTION_ARGS)
{
	NameData	application_name;
	NameData	recluster_name;
	int32		job_id;
	BgwPolicyRecluster *existing;
	Dimension  *dim;
	...
	if (!ts_is_hypertable(ht_oid))
		ereport(ERROR,
				(errcode(ERRCODE_TS_HYPERTABLE_NOT_EXIST),
				 errmsg("could not add recluster policy because \"%s\" is not a hypertable",
						get_rel_name(ht_oid))));
	...
	existing = ts_bgw_policy_recluster_find_by_hypertable(ts_hypertable_relid_to_id(ht_oid));

	if (existing != NULL)
	{
		if (!if_not_exists)
			ereport(ERROR, (errcode(ERRCODE_DUPLICATE_OBJECT), errmsg("recluster policy already exists for hypertable \"%s\"", get_rel_name(ht_oid))));

		if (!DatumGetBool(DirectFunctionCall2(nameeq, NameGetDatum(&existing->fd.hypertable_index_name), NameGetDatum(index_name))))
		{
			elog(WARNING, "could not add recluster policy due to existing policy on hypertable with different arguments");
			return -1;
		}
		/* If all arguments are the same, do nothing */
		ereport(NOTICE, (errmsg("recluster policy already exists on hypertable \"%s\", skipping", get_rel_name(ht_oid))));
		return -1;
	}

	/* Next, insert a new job into jobs table */
	namestrcpy(&application_name, "Recluster Background Job");
	namestrcpy(&recluster_name, "recluster");
	...
	job_id = ts_bgw_job_insert_relation(&application_name, &recluster_name, default_schedule_interval, DEFAULT_MAX_RUNTIME, DEFAULT_MAX_RETRIES, DEFAULT_RETRY_PERIOD);

	/* Now, insert a new row in the recluster args table */
	policy.fd.job_id = job_id;
	ts_bgw_policy_recluster_insert(&policy);

	PG_RETURN_INT32(job_id);
}

Datum
recluster_remove_policy(PG_FUNCTION_ARGS)
{
