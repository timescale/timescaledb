								 ...
								 NullableDatum refresh_newest_first)
{
	...
	CAggOverlapResult res = policy_refresh_cagg_check_for_overlaps(cagg, config, 0);
	switch (res)
	{
		case CAGG_OVERLAP_NONE:
			break;
		case CAGG_OVERLAP_EXACT:
			if (if_not_exists)
			{
				ereport(NOTICE,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			...
			/* else fallthrough and return error */
			TS_FALLTHROUGH;
		case CAGG_OVERLAP_NOT_EXACT:
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
