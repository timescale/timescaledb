									 ...
									 List *predicates, EState *estate, bool has_joins)
{
	...
	if (delete_only && ht_state->has_continuous_aggregate)
	{
		...
		invalidation_ctx.min_time_attno = compressed_column_metadata_attno(settings,
																	 chunk->table_id,
																	 chunk_time_attno,
																	 settings->fd.compress_relid,
																	 "min");
		invalidation_ctx.max_time_attno = compressed_column_metadata_attno(settings,
																	 chunk->table_id,
																	 chunk_time_attno,
																	 settings->fd.compress_relid,
																	 "max");
		...
		if (delete_only && summary == AllRowsPass)
		{
			...
			if (invalidation_ctx)
			{
				Datum min_time_datum =
					decompressor.compressed_datums[AttrNumberGetAttrOffset(invalidation_ctx->min_time_attno)];
				Datum max_time_datum =
					decompressor.compressed_datums[AttrNumberGetAttrOffset(invalidation_ctx->max_time_attno)];
				int64 batch_min =
			  ...
			  continuous_agg_invalidate_range(invalidation_ctx->hypertable_id, invalidation_ctx->chunk_relid, batch_min, batch_max);
			}
		}
		else
		{
