...
resize_affectedsegmentscxt(AffectedSegmentsCxt *as)
{
	...
	elog(LOG, "Input AffectedSegmentsCxt : %p", as);
	elog(LOG,
		 "Input AffectedSegmentsCxt::num_matching_uncompressedrows : %p",
		 as->num_matching_uncompressedrows);
	elog(LOG, "Input AffectedSegmentsCxt::compressed_tuples : %p", as->compressed_tuples);
	elog(LOG,
		 "Input AffectedSegmentsCxt::num_non_matching_compressedrows: %p",
	...
	if (!as->new_segments_found)
	{
		...
		elog(LOG, "new_cxt->num_matching_uncompressedrows allocated");
		new_cxt->compressed_tuples =
			repalloc(as->compressed_tuples, sizeof(unsigned long) * new_size);
		elog(LOG, "new_cxt->compressed_tuples allocated");
	...
	else
	{
		...
		elog(LOG, "new_cxt->num_non_matching_compressedrows allocated");
	...
	elog(LOG, "Output AffectedSegmentsCxt : %p", new_cxt);
	elog(LOG,
		 "Output AffectedSegmentsCxt::num_matching_uncompressedrows : %p",
		 new_cxt->num_matching_uncompressedrows);
	elog(LOG, "Output AffectedSegmentsCxt::compressed_tuples : %p", new_cxt->compressed_tuples);
	elog(LOG,
		 "Output AffectedSegmentsCxt::num_non_matching_compressedrows: %p",
		 new_cxt->num_non_matching_compressedrows);
	pfree(as);
	elog(LOG, "Input AffectedSegmentsCxt freed");
	return new_cxt;
}
					  ...
					  const int n_keys)
{
	...
	tuplestorecxt->tuplestore = tuplesort_begin_heap(RelationGetDescr(uncompressed_chunk_rel),
													 ...
													 NULL,
													 tuplesortopts);
	elog(LOG, "initialize_tuplestore start");
...
cleanup_tuplestorecxt(CompressTuplesortstateCxt *tuplestorecxt)
{
	if (tuplestorecxt)
	{
		...
		elog(LOG, "tuplesort_end finish");
		pfree(tuplestorecxt->sort_keys);
		elog(LOG, "tuplestorecxt->sort_keys finish");
		pfree(tuplestorecxt->sort_operators);
		elog(LOG, "tuplestorecxt->sort_operators finish");
		pfree(tuplestorecxt->sort_collations);
		elog(LOG, "tuplestorecxt->sort_collations finish");
		pfree(tuplestorecxt->nulls_first);
		elog(LOG, "tuplestorecxt->nulls_first finish");
		pfree(tuplestorecxt);
		elog(LOG, "tuplestorecxt finish");
		tuplestorecxt = NULL;
		elog(LOG, "cleanup_tuplestorecxt finish");
	}
}
 ...
 */
static void
process_new_segments(const RowDecompressor *decompressor, const Chunk *uncompressed_chunk,
					 Tuplesortstate *tuplestore, AffectedSegmentsCxt **affectedsegments,
					 const int nsegmentby_cols)
{
	elog(LOG, "process_new_segments start");
								  ...
								  NULL /*=abbrev*/))
	{
		...
		if (is_new_segment)
		{
			...
			{
				elog(LOG, "is_new_segment == true and resize_affectedsegmentscxt start");
				(*affectedsegments) = resize_affectedsegmentscxt((*affectedsegments));
				elog(LOG, "is_new_segment == true and resize_affectedsegmentscxt finish");
			}
	...
	elog(LOG, "process_new_segments finish");
}
 ...
 */
static void
save_affected_compressed_rows_using_indexscan(
	const RowDecompressor *decompressor, const RowCompressor *row_compressor,
	const Chunk *compressed_chunk, const Chunk *uncompressed_chunk,
	const CompressTuplesortstateCxt *uncompressed_tuplestorecxt,
	AffectedSegmentsCxt **affectedsegments, const int nsegmentby_cols)
{
	elog(LOG, "save_affected_compressed_rows_using_indexscan start");
	Relation index_rel;
	IndexScanDesc index_scan;
	 ...
	 */
	while (read_next_uncompressed_tuple(uncompressed_tuplestorecxt->tuplestore, uncompressed_slot))
	{
		Bitmapset *null_columns = NULL;
	...
	elog(LOG, "save_affected_compressed_rows_using_indexscan finish");
}
...
recompress_affected_segments(RowDecompressor *decompressor, RowCompressor *row_compressor,
							 AffectedSegmentsCxt *affectedsegments, const Chunk *uncompressed_chunk,
							 const int nsegmentby_cols,
							 const CompressTuplesortstateCxt *uncompressed_tuplestorecxt,
							 const ColumnCompressionInfo **keys, const int n_keys)
{
	elog(LOG, "recompress_affected_segments start");
	TupleTableSlot *compressed_slot = NULL;
	TupleTableSlot *uncompressed_slot = NULL;
	...
	{
		elog(LOG, "Start total_affected_compressed_rows : %lu", affectedsegments->total_affected_compressed_rows);
		ExecForceStoreHeapTuple(affectedsegments->compressed_tuples[compressed_row_count],
						   compressed_slot,
						   false);
		...
		{
			elog(LOG, "total_batches_to_decompress : %u ", total_batches_to_decompress);
		...
		elog(LOG, "compressed_row_count : %u ", compressed_row_count);
		affectedsegments->total_affected_compressed_rows--;
		elog(LOG, "Finish total_affected_compressed_rows : %lu ", affectedsegments->total_affected_compressed_rows);
	}
	ExecDropSingleTupleTableSlot(compressed_slot);
	elog(LOG, "recompress_affected_segments finish");
}
...
static void
recompress_new_segments(const RowDecompressor *decompressor, RowCompressor *row_compressor,
						AffectedSegmentsCxt *affectedsegments,
						const CompressTuplesortstateCxt *uncompressed_tuplestorecxt,
						const ColumnCompressionInfo **keys, const int n_keys)
{
	elog(LOG, "recompress_new_segments start");
	...
	ExecDropSingleTupleTableSlot(uncompressed_slot);
	elog(LOG, "recompress_new_segments finish");
}
/*
...
recompress_partial_chunks(const Chunk *uncompressed_chunk, const Chunk *compressed_chunk)
{
	...
	if (total_uncompressed_rows_in_chunk)
	{
		...
		elog(LOG, "First allocation of AffectedSegmentsCxt");
		AffectedSegmentsCxt *affectedsegments =
			(AffectedSegmentsCxt *) palloc0(sizeof(AffectedSegmentsCxt));
		elog(LOG, "First allocation of AffectedSegmentsCxt : %p", affectedsegments);
		affectedsegments->compressed_tuples =
			palloc0(sizeof(unsigned long) * MAX_ROWS_PER_COMPRESSION);
		elog(LOG,
			 "First allocation of AffectedSegmentsCxt::compressed_tuples : %p",
			 affectedsegments->compressed_tuples);
		affectedsegments->num_matching_uncompressedrows =
			palloc0(sizeof(unsigned long) * MAX_ROWS_PER_COMPRESSION);
		elog(LOG,
			 "First allocation of AffectedSegmentsCxt::num_matching_uncompressedrows : %p",
			 affectedsegments->num_matching_uncompressedrows);
		affectedsegments->num_non_matching_compressedrows =
			palloc0(sizeof(unsigned long) * MAX_ROWS_PER_COMPRESSION);
		elog(LOG,
			 "First allocation of AffectedSegmentsCxt::num_non_matching_compressedrows: %p",
			 affectedsegments->num_non_matching_compressedrows);
		...
		elog(LOG, "Last freeing of AffectedSegmentsCxt");
		/* freeup AffectedSegmentsCxt */
		pfree(affectedsegments->compressed_tuples);
		elog(LOG, "Freed AffectedSegmentsCxt::compressed_tuples %p", affectedsegments->compressed_tuples);
		pfree(affectedsegments->num_matching_uncompressedrows);
		elog(LOG, "Freed AffectedSegmentsCxt::num_matching_uncompressedrows%p", affectedsegments->num_matching_uncompressedrows);
		pfree(affectedsegments->num_non_matching_compressedrows);
		elog(LOG, "Freed AffectedSegmentsCxt::num_non_matching_compressedrows%p", affectedsegments->num_non_matching_compressedrows);
		pfree(affectedsegments);
		elog(LOG, "Freed AffectedSegmentsCx%p", affectedsegments);
