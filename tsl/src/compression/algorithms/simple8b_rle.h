...
simple8brle_compressor_partial_flush(Simple8bRleCompressor *compressor)
{
	...
	for (i = 0; i < num_buffered_elements; i++)
	{
		...
		if (bit_width * buffer_base[0].repcount >= SIMPLE8B_BITSIZE)
		{
			...
			{
				/* We need to flush the values as full 64bit blocks */
		...
		/* Truly lazy evaluation: use mask to minimize expensive calculations */
		...
		/* We can pack the elements - optimize hot path memory access */
		...
		{
			/* Cache buffer entry pointer to reduce repeated offset calculations */
			...
			/* Optimized inner loop - cache values and minimize memory access */
		...
		{
			/*
			 * Not enough values remaining to flush, move them at the beginnning and
			 * let the new cycle handle them. First I need to undo the decrement done
			 * in the packing loop.
			 */
	...
	/* If all elements were processed via continue statements, reset buffer */
...
simple8brle_compressor_append(Simple8bRleCompressor *compressor, uint64 val)
{
	...
	/* Optimized RLE detection: Cache buffer count to avoid repeated reads */
	uint32 num_buffered = compressor->num_buffered_elements;

	/* Fast path: Check for RLE against last element (most common case in time-series data) */
	if (likely(num_buffered > 0))
	{
		if (likely(compressor->last_value == val))
		{
			/* Direct pointer access to last entry - reduces address calculation overhead */
			Simple8bRleBuffer *last_entry = &compressor->uncompressed_buffer[num_buffered - 1];

			if (likely(val == last_entry->data))
			{
				/* RLE case: increment count, no new buffer entry needed */
				last_entry->repcount++;
				compressor->num_elements++;
				return; /* Early return - hottest path for time-series data */
			}
		}
	}

	/* Slow path: New unique value - simple buffering (no premature calculations) */
	Simple8bRleBuffer *new_entry = &compressor->uncompressed_buffer[num_buffered];

	/* Direct field assignment for optimal memory access pattern */
	new_entry->data = val;
	new_entry->repcount = 1;

	/* Update counters and cache in single operation group */
...
simple8brle_compressor_finish(Simple8bRleCompressor *compressor)
{
	...
	/* Validate element count by calculating actual elements in compressed blocks */
	uint32 calculated_elements = 0;
	uint32 min_possible_elements = 0;
	uint32 max_possible_elements = 0;

	BitArrayIterator selector_iter;
	bit_array_iterator_init(&selector_iter, &compressor->selectors);

	for (uint32 block_idx = 0; block_idx < compressor->compressed_data.num_elements; block_idx++)
	{
		uint8 selector = bit_array_iter_next(&selector_iter, SIMPLE8B_BITS_PER_SELECTOR);
		uint32 block_elements;

		if (simple8brle_selector_is_rle(selector))
		{
			/* RLE block: actual count is stored in the block data */
			uint64 block_data = compressor->compressed_data.data[block_idx];
			block_elements = simple8brle_rledata_repeatcount(block_data);
		}
		else
		{
			/* Bit-packed block: fixed number of elements per selector */
			block_elements = SIMPLE8B_NUM_ELEMENTS[selector];
		}

		calculated_elements += block_elements;

		if (block_idx < compressor->compressed_data.num_elements - 1)
		{
			/* Not the last block - contributes full count */
			min_possible_elements += block_elements;
			max_possible_elements += block_elements;
		}
		else
		{
			/* Last block - could have partial elements */
			if (simple8brle_selector_is_rle(selector))
			{
				/* RLE: exact count known */
				min_possible_elements += block_elements;
				max_possible_elements += block_elements;
			}
			else
			{
				/* Bit-packed: could be 1 to block_elements */
				min_possible_elements += 1;
				max_possible_elements += block_elements;
			}
		}
	}
...
simple8brle_bits_for_value(uint64 v)
{
	if (v == 0)
		return 0;
	return 64 - __builtin_clzll(v); // Single CPU instruction - no branches
}
#else
static inline uint32
simple8brle_bits_for_value(uint64 v)
