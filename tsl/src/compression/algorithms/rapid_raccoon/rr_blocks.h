 ...
 * values, the number of null values (RRCompressed struct in rr_internal.h).
 *
 ...
 * RRBlockType. PFOR, DFOR, and DICT add a trailing uint8 for a count
...
/*
 * RR_BT_RLE -- a run of one repeated value (simple RLE) or an
...
extern void rr_block_rle_init(RRBlockRLE *block, uint32 count, int64 base, int64 step,
							  fl_elem_width_t T);

static inline uint64
rr_block_rle_value_at(const RRBlockRLE *block, uint32 pos)
{
	return (uint64) block->base_value + (uint64) pos * (uint64) block->step_value;
}

/*
 * RR_BT_FOR -- a FastLanes body with an optional frame-of-reference
...
extern void rr_block_for_init(RRBlockFOR *block, uint32 count, int64 base, uint8 W,
							  fl_elem_width_t T, uint8 *body);

/*
 * RR_BT_PFOR -- PFOR with a base. The `body` packs the base-subtracted
...
extern void rr_block_pfor_init(RRBlockPFOR *block, uint32 count, int64 base, uint8 W, uint8 W_exc,
							   uint8 n_exc, const uint8 *positions, fl_elem_width_t T, uint8 *body,
							   uint8 *exc_body);

/*
 * RR_BT_DFOR -- strided deltas. the first count_base values are lane
...
extern void rr_block_dfor_init(RRBlockDFOR *block, uint32 count, uint16 S, int64 base,
							   uint8 W_bases, int64 delta_residual_base, uint8 W_delta_residuals,
							   bool zigzag, fl_elem_width_t T, uint8 *bases_body,
							   uint8 *delta_residual_body);

/*
 * RR_BT_DICT -- dictionary. `count_keys` distinct values form the `dict`
...
extern void rr_block_dict_init(RRBlockDICT *block, uint32 count, uint16 K, int64 key_base, uint8 W,
							   uint8 W_idx, bool zigzag_idxs, fl_elem_width_t T, uint8 *keys_body,
							   uint8 *idx_body);

typedef union RRBlockData
{
	RRBlockRLE rle;
	RRBlockFOR for_;
	RRBlockDFOR dfor;
...
/* For code simplicity RRBlock is used in both compression and decompression */
typedef struct RRBlock
{
	RRBlockType type;
	RRBlockData data;
...
extern void rr_free_blocks(RRBlock *block);
extern size_t rr_total_block_size(RRBlock *block);
extern bool rr_serialize_blocks(RRBlock *block, uint8 *dest, size_t max_size);

/*
 * scratch_buffer must be FL_MAX_ALIGNMENT-aligned with at least
 * RR_SCRATCH_BYTES - FL_MAX_ALIGNMENT usable bytes. most blocks stage their
 * packed input in the first RR_SCRATCH_STAGING_BYTES, and PFOR unpacks
 * its exception stream into the RR_PFOR_EXC_BYTES above that.
 */
extern size_t rr_decompress_blocks(uint32 num_blocks, StringInfo si, uint8 *dest,
 ...
 */
extern size_t rr_parse_one_block(RRBlock *block, StringInfo si, fl_elem_width_t T);
 ...
 * different blocks
 */
extern size_t rr_decompress_one_block(const RRBlock *block, uint8 *dest, uint8 *scratch_buffer,
									  fl_elem_width_t T);
