# RapidRaccoon integer compressor
...
The design goals of the RapidRaccoon (RR) compressor are partly
...
The RR compressor heavily utilizes the fastlanes library
...
1. when RR_RLE_CHECKPOINT (128) elements are buffered we check if all
values are the same, so the analysis can be skipped, because the best
encoding is RLE

2. at RR_DELTA_RLE_CHECKPOINT (32) elements we check if the subsequent
...
- `TY` - the unsigned type, a given generic function is parametrized with
- `STY` - the signed type, a given generic function is parametrized with
...
for the type. These include:
 ...
 - correct type semantics, bounds and arithmethics
