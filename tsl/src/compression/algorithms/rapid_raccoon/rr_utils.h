 ...
 */
static inline uint8
rr_signed_bytes_needed(int64 v)
{
	uint64 mag = (v < 0) ? (uint64) (~v) : (uint64) v;
	uint8 bits = (uint8) (rr_bit_width_u64(mag) + 1);
 ...
 */
static inline uint8
rr_block_header_scalar_len(int64 v)
{
	return (v == 0) ? 0 : rr_signed_bytes_needed(v);
}

/*  store the little endian low `n` bytes of `v` */
static inline void
rr_store_le64(uint8 *dst, uint64 v, uint8 n)
{
	for (uint8 i = 0; i < n; i++)
	{
		dst[i] = (uint8) (v >> (8 * i));
...
static inline void
rr_store_le32(uint8 *dst, uint32 v)
{
	for (uint8 i = 0; i < 4; i++)
	{
		dst[i] = (uint8) (v >> (8 * i));
...
static inline int64
rr_sign_extend_u64(uint64 v, uint8 width_bits)
{
...
static inline int64
rr_read_signed(const uint8 *src, uint8 n) /* n in 0..8 */
{
	if (n == 0)
	{
		return 0;
	...
	return rr_sign_extend_u64(v, (uint8) (8 * n));
}

/* copy uint64 words into little-endian stream order */
static inline void
rr_store_le64_words(uint8 *dst, const uint64 *src, size_t nbytes)
{
	...
	{
		rr_store_le64(dst + off, src[off / 8], 8);
	}
#endif
}

#define RR_DEFINE_ZIGZAG(TY)                                                                       \
	static inline TY rr_zigzag_encode_##TY(TY v)                                                   \
	{                                                                                              \
		TY sign = (TY) (v >> (sizeof(TY) * 8 - 1));                                                \
		return (TY) ((TY) (v << 1) ^ (TY) ((TY) 0 - sign));                                        \
	}                                                                                              \
	static inline TY rr_zigzag_decode_##TY(TY zz)                                                  \
	{                                                                                              \
		return (TY) ((TY) (zz >> 1) ^ (TY) ((TY) 0 - (TY) (zz & 1)));                              \
	}

RR_DEFINE_ZIGZAG(uint16)
RR_DEFINE_ZIGZAG(uint32)
RR_DEFINE_ZIGZAG(uint64)
#undef RR_DEFINE_ZIGZAG
