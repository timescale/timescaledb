...
static ChunkSortInfo *
compress_chunk_populate_sort_info(CompressionSettings *settings, Relation uncompressed_chunk_rel,
								  Relation compressed_chunk_rel, Relation index_rel,
								  const bool for_uncompressed)
{
	/* Variable declarations at beginning of scope */
	ChunkSortInfo *sort_info;
	int n;
	int position;
	const char *attname;
	...
	sort_info = palloc0(sizeof(ChunkSortInfo));

	/* Calculate array sizes */
	sort_info->num_segmentby = ts_array_length(settings->fd.segmentby);
	sort_info->num_orderby = ts_array_length(settings->fd.orderby);
	sort_info->n_keys = sort_info->num_segmentby + sort_info->num_orderby;

	/* Allocate arrays */
	sort_info->sort_keys = palloc(sizeof(*sort_info->sort_keys) * sort_info->n_keys);
	sort_info->sort_operators = palloc(sizeof(*sort_info->sort_operators) * sort_info->n_keys);
	sort_info->sort_collations = palloc(sizeof(*sort_info->sort_collations) * sort_info->n_keys);
	sort_info->nulls_first = palloc(sizeof(*sort_info->nulls_first) * sort_info->n_keys);
	sort_info->current_segment = palloc0(sizeof(CompressedSegmentInfo) * sort_info->n_keys);

	/* Populate sort information for each column */
	for (n = 0; n < sort_info->n_keys; n++)
	{
		if (n < sort_info->num_segmentby)
		{
			position = n + 1;
			attname = ts_array_get_element_text(settings->fd.segmentby, position);
			col_attno = get_attnum(chunk_rel->rd_id, attname);
			sort_info->current_segment[n].chunk_offset = AttrNumberGetAttrOffset(col_attno);
			sort_info->current_segment[n].segment_info =
				segment_info_new(TupleDescAttr(RelationGetDescr(chunk_rel),
											   sort_info->current_segment[n].chunk_offset));
		}
		else
		{
			position = n - sort_info->num_segmentby + 1;
		...
		compress_chunk_populate_sort_info_for_column(settings,
													 ...
													 attname,
													 &sort_info->sort_keys[n],
													 &sort_info->sort_operators[n],
													 &sort_info->sort_collations[n],
													 &sort_info->nulls_first[n]);
	}

	/* Allocate scankeys */
	sort_info->index_scankeys = palloc(sizeof(ScanKeyData) * sort_info->num_segmentby);
	sort_info->orderby_scankeys = palloc(sizeof(ScanKeyData) * sort_info->num_orderby * 2);

	/* Populate scankeys */
	create_segmentby_scankeys(settings, index_rel, compressed_chunk_rel, sort_info->index_scankeys);
	create_orderby_scankeys(settings, index_rel, compressed_chunk_rel, sort_info->orderby_scankeys);

	return sort_info;
}

static void
free_chunk_sort_info(ChunkSortInfo *sort_info)
{
	if (sort_info == NULL)
		return;

	if (sort_info->sort_keys)
		pfree(sort_info->sort_keys);
	if (sort_info->sort_operators)
		pfree(sort_info->sort_operators);
	if (sort_info->sort_collations)
		pfree(sort_info->sort_collations);
	if (sort_info->nulls_first)
		pfree(sort_info->nulls_first);
	if (sort_info->current_segment)
		pfree(sort_info->current_segment);
	if (sort_info->index_scankeys)
		pfree(sort_info->index_scankeys);
	if (sort_info->orderby_scankeys)
		pfree(sort_info->orderby_scankeys);
	pfree(sort_info);
}
...
recompress_chunk_segmentwise_impl(Chunk *uncompressed_chunk)
{
	...
	/* Need to populate sort info of uncompressed chunk */
	ChunkSortInfo *sort_info = compress_chunk_populate_sort_info(settings,
	...
	Tuplesortstate *input_tuplesortstate = tuplesort_begin_heap(uncompressed_rel_tupdesc,
																sort_info->n_keys,
																sort_info->sort_keys,
																sort_info->sort_operators,
																sort_info->sort_collations,
	...
	Tuplesortstate *recompress_tuplesortstate =
		tuplesort_begin_heap(uncompressed_rel_tupdesc,
							 sort_info->num_orderby,
							 &sort_info->sort_keys[sort_info->num_segmentby],
							 &sort_info->sort_operators[sort_info->num_segmentby],
							 &sort_info->sort_collations[sort_info->num_segmentby],
							 &sort_info->nulls_first[sort_info->num_segmentby],
							 maintenance_work_mem,
							 NULL,
							 false);
		 ...
		 found_tuple;)
	{
		...
		update_current_segment(sort_info->current_segment,
							   uncompressed_slot,
							   sort_info->num_segmentby);

		/* Build scankeys based on uncompressed tuple values */
		update_segmentby_scankeys(uncompressed_slot,
								  sort_info->current_segment,
								  sort_info->num_segmentby,
								  sort_info->index_scankeys);

		update_orderby_scankeys(uncompressed_slot,
								sort_info->current_segment,
								sort_info->num_segmentby,
								sort_info->num_orderby,
								sort_info->orderby_scankeys);

		index_rescan(index_scan, sort_info->index_scankeys, sort_info->num_segmentby, NULL, 0);
		...
		while (index_getnext_slot(index_scan, ForwardScanDirection, compressed_slot))
		{
			...
			while (result == Tuple_before)
			{
				 ...
				 */
				if (!found_tuple || check_changed_group(sort_info->current_segment,
														uncompressed_slot,
														sort_info->num_segmentby))
				{
		 ...
		 */
		while (!check_changed_group(sort_info->current_segment,
									uncompressed_slot,
									sort_info->num_segmentby))
		{
			tuples_for_recompression = true;
...
finish:
	...
	free_chunk_sort_info(sort_info);
...
static void
perform_recompression(ChunkSortInfo *sort_info, Relation compressed_chunk_rel,
					  ...
					  CompressionSettings *new_settings, Relation new_compressed_chunk_rel)
{
	...
	/* Initialize row compressor */
	row_compressor_init(&row_compressor,
						new_settings,
						RelationGetDescr(uncompressed_chunk_rel),
						RelationGetDescr(new_compressed_chunk_rel));

	/* Initialize writer and slot */
	...
	while (index_getnext_slot(index_scan, ForwardScanDirection, compressed_slot))
	{
		...
		else if (check_changed_group(sort_info->current_segment,
									 compressed_slot,
									 sort_info->num_segmentby))
		{
...
recompress_chunk_impl(Chunk *uncompressed_chunk)
{
	...
	/* Need to populate sort info of compressed chunk */
	ChunkSortInfo *sort_info = compress_chunk_populate_sort_info(settings,
																 uncompressed_chunk_rel,
																 compressed_chunk_rel,
																 index_rel,
	...
	if (ts_compression_settings_equal(new_settings, settings))
	{
		perform_recompression(sort_info,
							  compressed_chunk_rel,
							  uncompressed_chunk_rel,
							  index_rel,
							  new_settings,
	...
	ereport(DEBUG1,
			(errmsg("recompression %s for \"%s.%s\"",
					recompressed ? "successful" : "failed",
					NameStr(uncompressed_chunk->fd.schema_name),
					NameStr(uncompressed_chunk->fd.table_name))));
...
delete_tuple_for_recompression(Relation rel, ItemPointer tid, Snapshot snapshot)
{
	...
	result = table_tuple_delete(rel,
								tid,
								GetCurrentCommandId(true),
								snapshot,
								InvalidSnapshot,
								true /* for now, just wait for commit/abort, that might let us
										proceed */
								,
								&tmfd,
								true /* changingPart */);
 ...
 * We can only do this if there were no concurrent DML operations, so we check to see if there
 * are any uncompressed tuples in the chunk after compression. If there aren't, we can update
 * the chunk status
 *
