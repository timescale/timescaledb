...
recompress_chunk_segmentwise_impl(Chunk *uncompressed_chunk)
{
	...
	Tuplesortstate *segment_tuplesortstate = tuplesort_begin_heap(uncompressed_rel_tupdesc,
																  n_keys,
																  sort_keys,
																  sort_operators,
																  sort_collations,
	...
	Tuplesortstate *batch_tuplesortstate = tuplesort_begin_heap(uncompressed_rel_tupdesc,
																n_keys,
																sort_keys,
																sort_operators,
																sort_collations,
	...
	bool found_tuple =
		fetch_uncompressed_chunk_into_tuplesort(segment_tuplesortstate, uncompressed_chunk_rel);
	if (!found_tuple)
		goto finish;
	tuplesort_performsort(segment_tuplesortstate);

	for (found_tuple = tuplesort_gettupleslot(segment_tuplesortstate,
											  true /*=forward*/,
											  false /*=copy*/,
											  uncompressed_slot,
											  NULL /*=abbrev*/);
		 found_tuple;)
	{
		...
		while (index_getnext_slot(index_scan, ForwardScanDirection, compressed_slot))
		{
			...
			while (result == Tuple_before)
			{
				...
				tuplesort_puttupleslot(batch_tuplesortstate, uncompressed_slot);
				found_tuple = tuplesort_gettupleslot(segment_tuplesortstate,
													 true /*=forward*/,
													 false /*=copy*/,
													 uncompressed_slot,
													 NULL /*=abbrev*/);
			...
			if (done_with_segment)
			{
				...
				recompress_segment(batch_tuplesortstate, uncompressed_chunk_rel, &row_compressor);
				break;
			}
			...
			if (result == Tuple_match)
			{
				...
				row_decompressor_decompress_row_to_tuplesort(&decompressor, batch_tuplesortstate);
...
finish:
	...
	tuplesort_end(batch_tuplesortstate);
