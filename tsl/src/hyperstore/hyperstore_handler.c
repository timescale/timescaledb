...
static int32
get_compressed_chunk_id(int32 chunk_id)
{
	const FormData_chunk form = ts_chunk_get_formdata(chunk_id, true);
	return form.id > 0 ? form.compressed_chunk_id : -1;
}
								 ...
								 bool *compressed_relation_created)
{
	...
	hsinfo->compressed_relation_id = get_compressed_chunk_id(hsinfo->relation_id);
...
hyperstore_index_delete_tuples(Relation rel, TM_IndexDeleteOp *delstate)
{
	...
	compr_rel = table_open(hsinfo->compressed_relid, RowExclusiveLock);
	 ...
	 *
	 * Note that the function should return latestRemovedXid transaction ID,
	 * so need to remember those for each call and then return the latest
	 * removed of those.
	 */
									 ...
									 double *tups_recently_dead)
{
	...
	const CompressionSettings *settings = ts_compression_settings_get(hsinfo->compressed_relid);
	...
	if (indexInfo->ii_Predicate != NIL)
	{
		...
		foreach (lc, vars)
		{
			...
			if (!found)
			{
				...
				compress_iinfo.ii_IndexAttrNumbers[compress_iinfo.ii_NumIndexAttrs++] = cattno;
...
cleanup_compression_relations(void)
{
	if (cleanup_relids != NIL)
	{
		...
		foreach (lc, cleanup_relids)
		{
			...
			Oid compressed_relid = ts_chunk_get_relid(chunk->fd.compressed_chunk_id, false);

			ts_chunk_clear_compressed_chunk(chunk);
			ts_chunk_drop_by_relid(compressed_relid, DROP_RESTRICT, -1, false);
		}
...
convert_from_hyperstore(Oid relid)
{
	...
	/* There can be zero or one row to remove from the table. In a release
	 * build we will delete all, but that is fine. */
