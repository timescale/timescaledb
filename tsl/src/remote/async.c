...
/*
 * Create request and send a request. Note that we can only send one sql
 * statement per request. This is because we use PQsendQueryParams which uses
 * the extended query protocol instead of the simple one. The extended
 * protocol does not support multiple statements. In the future we can use a
 * PQsendQuery variant for queries without parameters, which can support
 * multiple statements because it uses the simple protocol. But this is an
