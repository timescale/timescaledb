...
/*
 * Build a [N][2] array where N is number of arguments and the inner array
 * is of [schema_name, type_name]
 */
static Datum
 ...
 *
 * Notes:
 *  - GROUP BY exprs always appear in the query's targetlist.
 *  - Some of the aggregates from the havingQual might also already appear in
 *    the targetlist.
 *  - We replace all existing entries with their corresponding entry from the
 *    modified targetlist.
 *  - If an aggregate (in the havingqual) does not exist in the TL, we create a
 *    materialization table column for it and use the finalize(column) form in
 *    the transformed havingQual.
 */
static Node *
create_replace_having_qual_mutator(Node *node, CAggHavingCxt *cxt)
{
	...
	/*
	 * Didn't find a match in targetlist. If it is an aggregate, create a
	 * partialize column for it in materialization hypertable and return
	 * corresponding finalize expr.
	 */
	if (IsA(node, Aggref))
	{
...
Node *
finalize_query_create_having_qual(FinalizeQueryInfo *inp, MatTableColumnInfo *mattblinfo)
{
...
*/
void
finalize_query_init(FinalizeQueryInfo *inp, Query *orig_query, MatTableColumnInfo *mattblinfo)
{
	AggPartCxt cxt;
	ListCell *lc;
	int resno = 1;
	...
	if (!inp->finalized)
		inp->final_havingqual = finalize_query_create_having_qual(inp, mattblinfo);
}
 ...
 */
Query *
finalize_query_get_select_query(FinalizeQueryInfo *inp, List *matcollist,
