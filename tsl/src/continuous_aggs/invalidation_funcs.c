...
Datum
invalidation_tuple_get_value(LogicalRepTupleData *tupleData, TupleDesc tupdesc, int attnum,
							 bool *isnull)
{
...
invalidation_tuple_get_heap_tuple(LogicalRepTupleData *tupleData, TupleDesc tupdesc)
{
	...
	for (int attnum = 1; attnum < _Anum_continuous_aggs_hypertable_invalidation_log_max; ++attnum)
		values[AttrNumberGetAttrOffset(attnum)] =
			invalidation_tuple_get_value(tupleData,
										 tupdesc,
										 attnum,
 ...
 */
LogicalRepRelId
invalidation_tuple_decode(LogicalRepTupleData *tuple, bytea *record)
{
	StringInfoData info;
...
ts_invalidation_read_record(PG_FUNCTION_ARGS)
{
	...
	invalidation_tuple_decode(&tupleData, raw_record);
