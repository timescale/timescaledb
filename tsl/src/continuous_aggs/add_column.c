...
continuous_agg_add_column(ContinuousAgg *cagg, AlterTableStmt *stmt)
{
	...
	/* The added column is materialized as NULL for buckets that were already
	 * materialized below the current watermark. Record that watermark so the
	 * planner does not rewrite and use the raw hypertable instead. If the
	 * watermark is at the type minimum, the cagg has no materialized data yet,
	 * so there is no need to block rewrite.
	 */
	int64 watermark = ts_cagg_watermark_get(cagg->data.mat_hypertable_id);
	if (!TS_TIME_IS_MIN(watermark, cagg->partition_type))
	{
		ts_continuous_agg_set_schema_change_timestamp(cagg->data.mat_hypertable_id, watermark);
	}
}
