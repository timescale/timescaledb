...
{
	char *orderby = build_order_by_clause(context);
...
execute_materializations(MaterializationContext *context)
{
	...
	SetConfigOption("timescaledb.ts_guc_enable_direct_compress_insert",
					"on",
					PGC_USERSET,
					PGC_S_SESSION);

	SetConfigOption("timescaledb.ts_guc_enable_direct_compress_insert_client_sorted",
					"on",
					PGC_USERSET,
					PGC_S_SESSION);
