	...
	switch (plan_type)
	{
		...
		case PLAN_TYPE_RANGES_PENDING:
		{
			...
			const int64 start_adjusted =
				int64_saturating_sub(context->internal_materialization_range.start, bucket_width);
			const int64 end_adjusted =
				int64_saturating_add(context->internal_materialization_range.end, bucket_width);
