 ...
 */
InternalTimeRange
continuous_agg_materialize_window_max(Oid timetype)
{
										   ...
										   int64 invalidate_prior_to_time)
{
	...
	ts_scanner_foreach(&iterator)
	{
		...
		InternalTimeRange maxwindow = continuous_agg_materialize_window_max(type);

		Assert(!lowest_isnull);
		Assert(!greatest_isnull);

		/* Need to handle infinite invalidations since bucketing cannot handle
		 * those values */
		if (lowest_modified_value == PG_INT64_MIN)
			entry_range.start = maxwindow.start;
		else
			entry_range.start =
