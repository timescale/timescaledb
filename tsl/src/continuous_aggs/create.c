					   ...
					   const ChunkInterval *chunk_interval)
{
	...
	time_dim_info = ts_dimension_info_create_open(mat_tbloid,
												  &mat_tbltimecol,
												  chunk_interval->value,
												  chunk_interval->type,
												  InvalidOid,
												  chunk_interval->origin,
			...
			ContinuousAggTimeBucketInfo *bucket_info, WithClauseResult *with_clause_options)
{
	...
	if (!with_clause_options[CreateMaterializedViewFlagChunkTimeInterval].is_default)
	{
		...
		matpartcol_interval.value = Int64GetDatum(interval_to_usec(DatumGetIntervalP(
	...
	else if (bucket_info->parent_mat_hypertable_id == INVALID_HYPERTABLE_ID)
	{
		...
		{
			Interval *interval = DatumGetIntervalP(matpartcol_interval.value);
			Interval *scaled = palloc(sizeof(Interval));
			scaled->month = interval->month * MATPARTCOL_INTERVAL_FACTOR;
			scaled->day = interval->day * MATPARTCOL_INTERVAL_FACTOR;
			scaled->time = interval->time * MATPARTCOL_INTERVAL_FACTOR;
			matpartcol_interval.value = IntervalPGetDatum(scaled);
		}
		else
		{
			int64 interval_usec = DatumGetInt64(matpartcol_interval.value);
			matpartcol_interval.value = Int64GetDatum(interval_usec * MATPARTCOL_INTERVAL_FACTOR);
		}
	}
