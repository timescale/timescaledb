					   ...
					   Oid chunk_relid)
{
	...
	if (!found)
	{
		hyper_inval_cache_entry->hypertable_id = cache_entry->hypertable_id;
		hyper_inval_cache_entry->watermark =
			get_lowest_invalidated_time_for_hypertable(cache_entry->hypertable_id);
	}
