...
multi_invalidation_state_init(MultiInvalidationState *state, const char *slot_name,
							  MemoryContext mcxt)
{
	state->context = AllocSetContextCreate(mcxt, "InvalidationsContext", ALLOCSET_DEFAULT_SIZES);
...
multi_invalidation_range_entry_init(MultiInvalidationRangeEntry *entry, int32 materialization_id,
									ContinuousAggsBucketFunction *bucket_function, Oid elemtyp)
{
	TypeCacheEntry *typcache =
		lookup_type_cache(get_multirange_type(elemtyp), TYPECACHE_MULTIRANGE_INFO);
	...
	entry->multirangetype = typcache->type_id;
	entry->rangetype = typcache->rngtype->type_id;
...
multi_invalidation_range_write(MultiInvalidationState *state, MultiInvalidationRangeEntry *entry)
{
	...
	TypeCacheEntry *typcache = lookup_type_cache(entry->multirangetype, TYPECACHE_MULTIRANGE_INFO);
	...
	MultirangeType *multirange = make_multirange(entry->multirangetype,
												 typcache->rngtype,
												 entry->ranges_count,
												 entry->ranges);
...
#if PG16_GE
	return construct_array_builtin(values, 2 * count, TEXTOID);
#else
	return construct_array(values, 2 * count, TEXTOID, -1, false, TYPALIGN_INT);
#endif
}
 ...
 * a GUC to control the batch size for processing.
 */
void
