					...
					const char *cagg_name, const bool is_cagg_create)
{
	 ...
	 * check only time based buckets since integer based bucks are always fixed. */
	bool time_offset_or_origin_set =
		(bucket_info.bucket_time_offset != NULL) || (bucket_info.bucket_time_origin > 0);

	/* Ignore time_bucket_ng in this check, since offset and origin was allowed in the past */
		...
		if (bucket_info.bucket_time_origin != bucket_info_parent.bucket_time_origin)
		{
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 ...
					 errdetail("Time origin of \"%s.%s\" [%s] should be the same as of \"%s.%s\" "
							   "[%s]",
							   cagg_schema,
							   cagg_name,
							   origin,
				...
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot create continuous aggregate with different bucket offset "
								"values"),
						 errdetail("Time offset of \"%s.%s\" [%s] should be the same as of "
								   "\"%s.%s\" [%s]",
								   cagg_schema,
								   cagg_name,
								   offset,
			...
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 ...
					 errdetail("Integer offset of \"%s.%s\" [" INT64_FORMAT
							   "] should be the same as of \"%s.%s\" [" INT64_FORMAT "]",
							   cagg_schema,
							   cagg_name,
