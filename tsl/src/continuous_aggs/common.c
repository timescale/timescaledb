					...
					const char *cagg_name, const bool is_cagg_create)
{
	...
	if (!ts_cagg_query_supported(query, hint, detail, finalized, for_rewrites))
	{
		ereport(ERROR,
	...
	part_dimension = ts_cagg_hypertable_dim_supported(ht_rte, ht, msg, hint, detail, for_rewrites);
	if (!part_dimension)
	{
		ts_cache_release(&hcache);
		ereport(ERROR,
