...
range. When using the hypertable invalidation log table, this happens
by installing a trigger on the source hypertable when the first
continuous aggregate on that hypertable is created.
...
`hypertable_plugin.c` and is used to read hypertable invalidations
