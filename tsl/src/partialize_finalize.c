					  ...
					  bool serialized_isnull, bool *deserialized_isnull)
{
	...
	*deserialized_isnull = true;
	if (OidIsValid(combine_meta->deserialfnoid))
	{
		...
		/*
		 * For testing, we print out the memory context before and after the
		 * block.
		 *
		 * It would be nice to print the resource owner name here as well to
		 * make sure that it is the same as before, but this is hidden inside
		 * resowner.c.
		 */
		ereport(DEBUG2,
				errmsg("%s - deserializing partial", __func__),
				errdetail("mcxt: %s", CurrentMemoryContext->name));
		...
		PG_CATCH();
		{
			 ...
			 * when using elog()), the format is in the old numeric and we can
			...
			CurrentResourceOwner = oldowner;
			ereport(DEBUG2,
					errmsg("%s - attempting repair of serialized partial", __func__),
					errdetail("mcxt: %s", CurrentMemoryContext->name));
			serialized_partial =
