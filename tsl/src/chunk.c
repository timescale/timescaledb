...
{
	const char *multidim_allowed =
		GetConfigOption("timescaledb.merge_chunks_multidim_allowed", true, false);

	if (multidim_allowed == NULL)
		return false;

	return (strcasecmp("on", multidim_allowed) == 0 || strcasecmp("true", multidim_allowed) == 0);
}
