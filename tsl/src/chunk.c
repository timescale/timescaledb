...
typedef struct SplitRelationInfo
{
	...
	Oid tmp_heap_relid; /* The relid of a temporary heap for relations getting a heap swap */
...
route_tuple(TupleTableSlot *slot, const SplitPoint *sp)
{
	...
	Assert(!isnull);
	int64 point = ts_time_value_to_internal(value, sp->type);
...
route_compressed_tuple(TupleTableSlot *slot, const SplitPoint *sp)
{
	...
	if (sp->point >= min_point && sp->point < max_point)
		return -1;
	else if (min_point < sp->point)
		return 0;

	Assert(max_point >= sp->point);
	return 1;
}
...
route_next_compressed_tuple(TupleTableSlot *slot, SplitContext *scontext, int *routing_index)
{
	...
	else if (scontext->rws_index >= 0)
	{
		...
		/* Copy over visibility information from the original segment tuple */
...
copy_tuples_for_split(SplitContext *scontext)
{
	...
	while (table_scan_getnextslot(scan, ForwardScanDirection, srcslot))
	{
		 ...
		 * that segment.
		 */
		while ((tuple = sp->route_next_tuple(srcslot, scontext, &routingindex)))
		{
			...
			else
			{
				...
				reform_and_rewrite_tuple(tuple, srcrel, rws);
			}
		}
	}
