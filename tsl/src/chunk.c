 ...
 *
 * To be able to merge, check that:
 *
...
chunk_merge_chunks(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < nrelids; i++)
	{
		...
		if (chunk->fd.compressed_chunk_id != INVALID_CHUNK_ID && !ts_is_hypercore_am(chunk->amoid))
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					 errmsg("cannot merge compressed chunks not using hypercore access method"),
					 errhint("Decompress the chunks first or convert to Hypercore TAM.")));
		...
		/* Need to check that storage is compatible across the chunks being
		 * merged. It is not possible to merge two chunks with different
		 * storage, except for Hypercore TAM and Heap TAM, which are
		 * compatible (since both using heap storage). */
