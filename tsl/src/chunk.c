...
relation_split_info_create(Relation splitrel, Relation rel, struct VacuumCutoffs *cutoffs)
{
	...
	rsi->tupmap = convert_tuples_by_name(RelationGetDescr(splitrel), RelationGetDescr(rel));
...
{
	TupleDesc oldTupDesc = RelationGetDescr(splitrel);
	TupleDesc newTupDesc = RelationGetDescr(rsi->rel);
	HeapTuple tupcopy;
 ...
 *
 * The column/dimension and split_at point are optional. If these arguments
 * are not specified, the chunk is split in two equal sizes (from a range
 * perspective) based on the primary partitioning column.
 */
Datum
chunk_split_chunk(PG_FUNCTION_ARGS)
{
	...
	if (!object_ownercheck(RelationRelationId, relid, GetUserId()))
		aclcheck_error(ACLCHECK_NOT_OWNER,
					   get_relkind_objtype(splitrel->rd_rel->relkind),
					   get_rel_name(relid));

	/* Lock toast table to prevent it from being concurrently vacuumed */
	if (splitrel->rd_rel->reltoastrelid)
		LockRelationOid(splitrel->rd_rel->reltoastrelid, AccessExclusiveLock);
	...
	/* Get the dimension by name, or if not specified use the primary dimension */
	if (colname != NULL)
	{
		...
		if (argtype != splitdim_type)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 ...
					 errdetail("The argument type must much the dimension \"%s\"",
	...
	struct VacuumCutoffs cutoffs;
	char relpersistence = splitrel->rd_rel->relpersistence;
	...
	while (table_scan_getnextslot(scan, ForwardScanDirection, srcslot))
	{
		...
		num_tuples++;
		reform_and_rewrite_tuple(tuple, splitrel, rsi);
	}
