...
my $query = q[CREATE OR REPLACE PROCEDURE custom_proc_sleep60(jobid int, config jsonb) LANGUAGE PLPGSQL AS 
$$
BEGIN
...
my $query_add = q[select add_job('custom_proc_sleep60', '5 minutes', initial_start => now())];
...
my $query_pid = qq[select pid from pg_stat_activity where application_name like 'User-Defined Action%' and query like '%custom_proc_sleep60%'];
...
my $errlog = $node->safe_psql('postgres', 'select count(*) from _timescaledb_internal.job_errors where job_id = 1000 and error_data is null');
