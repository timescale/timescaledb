...
$result = $s1->query_safe("SET timescaledb.compress_truncate_behaviour TO truncate_or_delete;");
...
$result = $s1->query_safe("SELECT compression_status, count(*) FROM chunk_compression_stats('sensor_data') GROUP BY 1 ORDER BY 1, 2;");
...
$result = $s1->query_safe("SELECT relation::regclass::text FROM pg_locks WHERE granted AND relation::regclass::text LIKE '%hyper%chunk' AND locktype = 'relation' AND mode = 'AccessExclusiveLock' ORDER BY relation, locktype;");
