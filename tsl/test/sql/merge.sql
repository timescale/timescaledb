...
CREATE TABLE target (
   ...
   to_be_dropped text
) WITH (tsdb.hypertable, tsdb.chunk_interval='5 seconds');
...
SELECT compress_chunk(i) FROM show_chunks('target') i;
...
generate_series(1,4) as location;
SELECT compress_chunk(i) FROM show_chunks('SOURCE') i;
...
ALTER TABLE target SET (timescaledb.compress);
...
ALTER TABLE source SET (timescaledb.compress);
