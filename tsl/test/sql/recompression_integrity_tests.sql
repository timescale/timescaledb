...
DECLARE
    ...
    sql             text;
BEGIN
    temp_table_name := 'temp_' || replace(p_chunk::text, '.', '_');

    -- Get row counts
    EXECUTE format('SELECT COUNT(*) FROM %s', p_chunk) INTO current_count;

    EXECUTE format('SELECT COUNT(*) FROM %I', temp_table_name) INTO original_count;
    ...
    sql := format($f$
        SELECT COUNT(*) FROM (
            (SELECT * FROM %s EXCEPT SELECT * FROM %I)
            UNION ALL
            (SELECT * FROM %I EXCEPT SELECT * FROM %s)
        ) AS differences
    $f$, p_chunk, temp_table_name, temp_table_name, p_chunk);

    EXECUTE sql INTO diff_count;
...
BEGIN
    ...
    temp_table_name := 'temp_' || replace(old_chunk_name::text, '.', '_');

    -- Create temp table with same structure as source
    stmt := format($f$
        CREATE TEMP TABLE %I AS
        SELECT * FROM %s WITH NO DATA
    $f$, temp_table_name, old_chunk_name);
    EXECUTE stmt;
...
DROP FUNCTION IF EXISTS test_recompression(text);
