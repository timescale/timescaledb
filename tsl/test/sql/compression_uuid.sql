...
SELECT
  to_char(to_timestamp(u), 'YYYY-MM-DD:HH24:MI:SS'),
  uuid_version(u),
  count(*)
FROM
  uuids
...
-- the test flaky
  to_timestamp(u) > '2000-01-01:01:01:01'::timestamptz OR
  uuid_version(u) >= 1
GROUP BY 1
ORDER BY 1;
...
SELECT
  ...
  count(*) FILTER (WHERE u < to_uuidv7_zeroed('Wed Jun 25 08:16:46.348 2025 PDT')) AS count_below_on_uuid,
  count(*) FILTER (WHERE to_timestamp(u) < 'Wed Jun 25 08:16:46.348 2025 PDT') AS count_below_on_time,
  count(*) FILTER (WHERE u >= to_uuidv7('Wed Jun 25 08:16:46.348 2025 PDT', true)) AS count_above_on_uuid,
  count(*) FILTER (WHERE to_timestamp(u) >= 'Wed Jun 25 08:16:46.348 2025 PDT') AS count_above_on_time
FROM
  uuids
WHERE
  uuid_version(u) = 7;
...
FROM
  (
    SELECT
      u, ts, to_timestamp_micros(u) ts2
    FROM
      subms
  ) x
WHERE
...
WHERE id >= to_uuidv7_zeroed('Wed Aug 13 2025 PDT') AND
      id < to_uuidv7_zeroed('Wed Aug 16 2025 PDT');
