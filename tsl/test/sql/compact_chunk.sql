...
-- overlap merge pulls the NULL rows out into their own batch. A mixed-null
-- batch that overlaps nothing is already ordered and is left untouched.
...
-- two batches overlap and the merge pulls the NULLs out into their own batch.
...
-- compact_chunk must detect the overlap and pull the NULLs into their own batch
SELECT _timescaledb_functions.compact_chunk(chunk) FROM show_chunks('metrics_mixed_nulls') chunk;

-- After compaction: Batch 2 and Batch 3 merged, NULLs in their own batch
-- (meta_min IS NULL) at the end (NULLS LAST) rather than between 1800 and 1801.
SELECT ctid, device, _ts_meta_count, _ts_meta_min_1, _ts_meta_max_1
FROM :MIXED_NULLS_CHUNK
...
-- compact_chunk skips legacy chunks whose orderby column has no firstlast sparse
-- index. Compaction reads exact batch boundaries from firstlast metadata, so
-- without it the chunk is left untouched (UNORDERED stays set). Configuring minmax
-- (and not firstlast) on the orderby column reproduces a legacy chunk: a
-- user-supplied index for a column suppresses the firstlast that orderby columns
-- get by default.
CREATE TABLE metrics_no_firstlast (time TIMESTAMPTZ NOT NULL, device TEXT, value float)
...
FROM pg_attribute
WHERE attrelid = :'NO_FL_CHUNK'::regclass AND attnum > 0;

-- Overlapping batches; chunk is UNORDERED.
SELECT ctid, _ts_meta_count, _ts_meta_min_1, _ts_meta_max_1
FROM :NO_FL_CHUNK ORDER BY _ts_meta_min_1;
SELECT DISTINCT _timescaledb_functions.chunk_status_text(chunk) FROM show_chunks('metrics_no_firstlast') chunk;

-- compact_chunk is a no-op and warns with a hint on how to enable it: there is
-- no firstlast metadata to read batch boundaries from.
\set VERBOSITY default
SELECT _timescaledb_functions.compact_chunk(chunk) FROM show_chunks('metrics_no_firstlast') chunk;
\set VERBOSITY terse

-- Batches unchanged (same ctids) and UNORDERED is still set.
SELECT ctid, _ts_meta_count, _ts_meta_min_1, _ts_meta_max_1
FROM :NO_FL_CHUNK ORDER BY _ts_meta_min_1;
SELECT DISTINCT _timescaledb_functions.chunk_status_text(chunk) FROM show_chunks('metrics_no_firstlast') chunk;
