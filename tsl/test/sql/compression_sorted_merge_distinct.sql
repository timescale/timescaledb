...
explain (buffers off, costs off) select * from t order by ts;
explain (buffers off, costs off) select * from t where low_card = 1 order by ts;
explain (buffers off, costs off) select * from t where high_card = 1 order by ts;
explain (buffers off, costs off) select * from t where low_card = 1 and high_card = 1 order by ts;
...
explain (buffers off, costs off) select * from t where high_card < 500 order by ts;
