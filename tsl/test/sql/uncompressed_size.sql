...
SELECT compress_chunk(chunk) FROM show_chunks('t1') AS chunk;
SELECT compress_relid AS "COMPRESSED_RELATION" from show_chunks('t1') chunk JOIN _timescaledb_catalog.compression_settings cs ON cs.relid=chunk ORDER BY chunk::text DESC LIMIT 1 \gset

SELECT _timescaledb_functions.compressed_data_info(time), _timescaledb_functions.compressed_data_info(device), _timescaledb_functions.compressed_data_info(value) FROM :COMPRESSED_RELATION;
SELECT * FROM _timescaledb_functions.estimate_uncompressed_size(:'COMPRESSED_RELATION');
