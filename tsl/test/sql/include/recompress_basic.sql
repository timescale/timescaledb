...
FROM :COMP_CHUNK_NAME c  WHERE b = 11 group by b;

SELECT count(*) from test3;
SELECT count(*) from :COMP_CHUNK_NAME;
...
SELECT * FROM
(( SELECT * from test4) EXCEPT (select * from test3) ) q;

SELECT * FROM
(( SELECT * from test3) EXCEPT (select * from test4) ) q;
