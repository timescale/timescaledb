...
CREATE OR REPLACE FUNCTION _timescaledb_internal.rapid_raccon_append_bigint(internal, BIGINT)
   RETURNS internal
   AS :MODULE_PATHNAME, 'ts_rapid_raccoon_compressor_append'
   LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timescaledb_internal.rapid_raccon_append_timestamptz(internal, timestamptz)
   RETURNS internal
   AS :MODULE_PATHNAME, 'ts_rapid_raccoon_compressor_append'
   LANGUAGE C IMMUTABLE PARALLEL SAFE;

CREATE OR REPLACE FUNCTION _timescaledb_internal.rapid_raccoon_finish(internal)
   RETURNS _timescaledb_internal.compressed_data
   AS :MODULE_PATHNAME, 'ts_rapid_raccoon_compressor_finish'
   LANGUAGE C IMMUTABLE PARALLEL SAFE STRICT;
...
CREATE AGGREGATE _timescaledb_internal.compress_rapid_raccoon(BIGINT) (
    STYPE = internal,
    SFUNC = _timescaledb_internal.rapid_raccon_append_bigint,
    FINALFUNC = _timescaledb_internal.rapid_raccoon_finish
);

CREATE AGGREGATE _timescaledb_internal.compress_rapid_raccoon(timestamptz) (
    STYPE = internal,
    SFUNC = _timescaledb_internal.rapid_raccon_append_timestamptz,
    FINALFUNC = _timescaledb_internal.rapid_raccoon_finish
);
