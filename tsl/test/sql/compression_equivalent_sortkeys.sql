...
-- Test that batch sorted merge does not crash
CREATE TABLE test2 (s1 int4, s2 int4, s3 int2, o1 int4);
SELECT FROM create_hypertable('test2', 'o1');
ALTER TABLE test2 SET (timescaledb.compress_segmentby = 's1, s2, s3');

INSERT INTO test2 SELECT t1, t1, t4, t3 * 10 FROM generate_series(1, 2) AS t1, generate_series(1, 2) AS t3, generate_series(1, 4) AS t4;
SELECT count(compress_chunk(ch)) FROM show_chunks('test2') AS ch;
...
-- Cross-type equivalency for Batch sorted merge scenario flagged by LLM:
-- cannot use non-vars as sort keys for batch sorted merge as the heap is not set up for it
CREATE TABLE cross_type_repro (
    seg1 int NOT NULL,
    seg2 int NOT NULL,
    ob1 int NOT NULL,
    num_col numeric NOT NULL
);

SELECT FROM create_hypertable('cross_type_repro', 'ob1', chunk_time_interval => 1000);

ALTER TABLE cross_type_repro SET (
    timescaledb.compress,
    timescaledb.compress_segmentby = 'seg1, seg2',
    timescaledb.compress_orderby = 'ob1'
);

INSERT INTO cross_type_repro
    SELECT g % 3, g % 5, g, (g % 3)::numeric
    FROM generate_series(1, 50) g;

SELECT count(compress_chunk(c)) FROM show_chunks('cross_type_repro') c;

SET timescaledb.debug_require_batch_sorted_merge TO 'forbid';

-- seg1 (int) = num_col (numeric) implicitly casts seg1 to numeric, cannot use (seg1, ob1) for BSM
SELECT * FROM cross_type_repro WHERE seg1 = num_col ORDER BY num_col, ob1;

-- ob1 (int) = num_col (numeric) implicitly casts ob1 to numeric, cannot use ob1 for BSM
SELECT * FROM cross_type_repro WHERE ob1 = num_col ORDER BY num_col;

RESET timescaledb.debug_require_batch_sorted_merge;

drop table cross_type_repro cascade;
