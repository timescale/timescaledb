...
-- ============================================================================
-- Part 2: simulate OOM from dshash_find_or_insert
-- ============================================================================
...
-- No NOTICE from here on: the first throw left a marker, so lookup returns
-- DISABLED
...
-- ============================================================================
-- Part 3: OOM during the seqnum-seed catalog scan
-- ============================================================================
-- The resolver runs in the DML path inside an internal subtransaction.  A throw
-- during its max_seqnum scan holds a catalog relation + snapshot; the
-- subtransaction rollback releases them cleanly, so the INSERT commits with no
-- resource-leak WARNINGs and a later INSERT does not hang.
...
CREATE MATERIALIZED VIEW gauge_daily
  ...
  GROUP BY bucket, sensor_id
  WITH NO DATA;
ALTER MATERIALIZED VIEW gauge_daily SET (timescaledb.enable_granular_refresh = true);

SELECT debug_waitpoint_enable('tenant_tracker_seqnum_scan_oom');

-- First touch: the resolver's max_seqnum scan throws mid-scan; the subtransaction
...
SELECT debug_waitpoint_release('tenant_tracker_seqnum_scan_oom');

-- Refresh falls back to the full invalidation log and materializes correctly.
CALL refresh_continuous_aggregate('gauge_daily', '2020-01-01 00:00+00', NULL);
SELECT sensor_id, bucket, avg FROM gauge_daily ORDER BY sensor_id, bucket;
