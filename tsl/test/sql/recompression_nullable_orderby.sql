...
CREATE TABLE t1(time int, v1 int);
-- t1 is declared NOT NULL on a hypertable as partitioning column
SELECT create_hypertable('t1', 'time', chunk_time_interval => 10000);
ALTER TABLE t1 SET (timescaledb.compress, timescaledb.compress_orderby='v1');

INSERT INTO t1 SELECT 1, g FROM generate_series(1,800) g;
INSERT INTO t1 SELECT 1, NULL FROM generate_series(1,200);
SELECT compress_chunk(show_chunks('t1'));

-- Now this chunk is partial
INSERT INTO t1 SELECT 1, g FROM generate_series(901,1400) g;

-- Should get a notice about not recompressing because of nullable order by
...
SELECT count(*) AS wrong_rows FROM (
  SELECT v1, lead(v1) OVER (ORDER BY v1) AS next FROM t1 ORDER BY v1
...
-- Use a function to recompress-sgementwise
-- Should get a notice about not recompressing because of nullable order by
