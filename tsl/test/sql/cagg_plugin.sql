...
-- Test the invalidation plugin and make sure it can return reasonable
-- data. We do that by creating a hypertable and then registering a
-- logical decoding slot on it and using the plugin to get the
-- changes.
...
WITH
  changes AS (
      SELECT (_timescaledb_functions.cagg_read_invalidation_record(data)).*
...
CREATE TABLE conditions_index (
       ...
       temp FLOAT,
       UNIQUE (recorded_at, device_id)
);

ALTER TABLE conditions_index
      REPLICA IDENTITY USING INDEX conditions_index_recorded_at_device_id_key;

SELECT create_hypertable('conditions_index', 'recorded_at');

INSERT INTO conditions_index VALUES
       ('2020-05-02 12:34:56Z', 42, 37.0),
       ('2020-05-02 13:45:00Z', 42, 37.0);

SELECT * FROM get_invalidations('my_slot', 'conditions_index', 'recorded_at');
