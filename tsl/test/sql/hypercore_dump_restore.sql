...
select setseed(0.3);
create table hyperdump (time timestamptz, device int, tempc float, tempf float generated always as (tempc * 2 + 30) stored, status text default 'idle');
...
select compress_chunk(ch, compress_using=>'hypercore') from show_chunks('hyperdump') ch;
