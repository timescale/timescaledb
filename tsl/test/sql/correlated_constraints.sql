...
\set ON_ERROR_STOP 0
SELECT * FROM add_dimension('sample_table', 'name', correlated => true);
\set ON_ERROR_STOP 1

-- Specify a correlated constraint
SELECT * FROM add_dimension('sample_table', 'sensor_id', correlated => true);
...
SELECT id AS dimension_id FROM _timescaledb_catalog.dimension WHERE correlated \gset
-- should show MIN_INT/MAX_INT entries
SELECT * FROM _timescaledb_catalog.dimension WHERE correlated AND id = :dimension_id;
