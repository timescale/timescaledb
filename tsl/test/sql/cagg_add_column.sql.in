...
-- before ADD COLUMN there is no schema change recorded
SELECT schema_change_timestamp IS NULL AS schema_change_unset
FROM _timescaledb_catalog.continuous_agg WHERE user_view_name='cagg_rt';

ALTER MATERIALIZED VIEW cagg_rt
    ADD COLUMN min_temp double precision GENERATED ALWAYS AS (min(temp)) STORED;

-- ADD COLUMN records the materialization watermark at the time of the change
SELECT schema_change_timestamp IS NOT NULL AS schema_change_set,
       schema_change_timestamp = _timescaledb_functions.cagg_watermark(mat_hypertable_id) AS matches_watermark
FROM _timescaledb_catalog.continuous_agg WHERE user_view_name='cagg_rt';
...
-- 1-day chunk interval so the 3-day refresh below produces 3 chunks
CREATE MATERIALIZED VIEW cagg_e2e
WITH (timescaledb.continuous,
      timescaledb.materialized_only=true,
      timescaledb.chunk_interval = '1 day') AS
