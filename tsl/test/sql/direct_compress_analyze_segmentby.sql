...
----------------------------------------------------------------------
-- Test: Too few rows — analysis skipped (tuples_to_sort < tuple_sort_limit)
----------------------------------------------------------------------
BEGIN;
...
-- Only ~7205 rows (1441 minutes * 5 devices), well under the 30000 sort limit
...
-- fewer than batch_size_limit (1000) rows
----------------------------------------------------------------------
BEGIN;
...
-- segmentby should be NULL — device_id=5 has ~301 rows, below batch_size_limit
SELECT * FROM _timescaledb_catalog.compression_settings;
ROLLBACK;
