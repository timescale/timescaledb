...
setup {
   ...
   SELECT time + (INTERVAL '1 minute' * random()) AS time, sensor_id, 1 as nullable,
   random() AS cpu,
   ...
   timescaledb.compress_orderby = 'time,nullable');
...
step "s2_insert_do_nothing" {
   INSERT INTO sensor_data
   VALUES ('2022-01-01 20:00'::timestamptz, 1, 1, 1.0, 1.0), ('2022-01-01 21:00'::timestamptz, 2, 1, 2.0, 2.0) 
   ON CONFLICT (time, sensor_id) DO NOTHING;
}

step "s2_insert_existing_do_nothing" {
   INSERT INTO sensor_data 
   SELECT time, sensor_id, 1, 1.0, 1.0 FROM sensor_data
   WHERE sensor_id = 4
   LIMIT 1
   ON CONFLICT (time, sensor_id) DO NOTHING;
}
...
step "s2_upsert" {
   INSERT INTO sensor_data 
   VALUES ('2022-01-01 20:00'::timestamptz, 100, 1, 9999, 9999), ('2022-01-01 21:00'::timestamptz, 101, 1, 9999, 9999) 
   ON CONFLICT (time, sensor_id) DO UPDATE SET cpu = EXCLUDED.cpu, temperature = EXCLUDED.temperature;
}

step "s2_upsert_existing" {
   INSERT INTO sensor_data 
   SELECT time, sensor_id, 1, 9999, 9999 FROM sensor_data
   WHERE sensor_id = 4
   LIMIT 1
   ON CONFLICT (time, sensor_id) DO UPDATE SET cpu = EXCLUDED.cpu, temperature = EXCLUDED.temperature;
}
