...
setup
{
    ...
    declare
        chunks_arr regclass[];
    begin
        --select array_agg(ch) into chunks_arr from show_chunks(hypertable) ch;
    ...
    declare
        chunk regclass;
    begin
        select ch into chunk from show_chunks(hypertable) ch limit 1;
...
step "s1_show_chunks" { select show_chunks('readings'); }
step "s1_show_data" {
...
step "s2_show_chunks" { select show_chunks('readings'); }
...
step "s3_show_chunks" { select show_chunks('readings'); }
