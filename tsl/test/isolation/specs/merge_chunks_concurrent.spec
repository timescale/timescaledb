...
# Waitpoints
session "wp"
step "wp_before_rewrite_on" { SELECT debug_waitpoint_enable('merge_chunks_before_rewrite'); }
step "wp_before_rewrite_off" { SELECT debug_waitpoint_release('merge_chunks_before_rewrite'); }
...
step "s6_merge_1_3_concurrently" {
    call merge_chunks(getchunk('readings', 0), getchunk('readings', 2), concurrently => true);
}
...
# Test concurrent merges that are different but has the same result chunk
permutation "wp_before_rewrite_on" "s1_show_chunks" "s5_merge_1_2_concurrently" "s6_merge_1_3_concurrently" "wp_before_rewrite_off" "s1_show_chunks"
