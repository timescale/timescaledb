...
#
# Four scenarios, one shared hypertable + cagg.  The first two are described
# here, the last two above their permutations:
#
...
#
#   3. The late-arrival window changes between a writer's drain and its
#      invalidation stamping.
#
#   4. Two transactions each change one offset, so the second must read what the
#      first wrote rather than the state both started from.
#
...
# The offsets as stored in the catalog.
step "v_offsets"
{
    SELECT granular_refresh_start_offset AS start_offset,
           granular_refresh_end_offset   AS end_offset
    FROM _timescaledb_catalog.hypertable_cagg_settings
    WHERE hypertable_id = (
        SELECT id FROM _timescaledb_catalog.hypertable WHERE table_name = 'conditions');
}
step "v_tracking"
{
...
# Scenario 4: two transactions change one offset each.
#
# From start = '36500 days', end = '1 day', A sets start = '3 days' and A2 sets
# end = '4 days'.  Either is valid by itself at the start but would result in
# invalid window (start > end) when both are run. Check that the second one
# errors out correctly instead of writing an invalid setting.
#
# A2 blocks on the tuple lock A holds on the row.  Once A commits, the scan
# follows the update chain (TUPLE_LOCK_FLAG_FIND_LAST_VERSION) and A2 reads the
# committed '3 days', so it fails with "invalid granular refresh window".
#
# v_offsets is what tells the two reads apart: end is still '1 day', so A2 wrote
# nothing, and start is '3 days', so A's write survived whole.  A stale read
# would have validated against '36500 days' and left '3 days | 4 days'.
permutation "a_begin" "a2_begin" "a_start_3d" "a2_end_4d" "a_commit" "a2_commit" "v_offsets"
