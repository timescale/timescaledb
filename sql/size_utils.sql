...
DECLARE
  ...
  v_columns integer;
  v_varlen_query text:= '';
  v_multiplier decimal:=1.2; -- multiplier to account for page fill factor and alignment padding
BEGIN
  ...
  IF v_num_varlen_columns > 0 THEN
	  SELECT ' + (' || string_agg(format('sum(_timescaledb_functions.compressed_data_column_size(%I,NULL::%s))', attname, atttypid::regtype), ' + ') || ')' FROM pg_attribute INTO v_varlen_query WHERE attrelid = v_uncompressed_chunk AND attnum > 0 AND NOT attisdropped AND attlen = -1;
  END IF;
  ...
  FOR v_index, v_varlen_query, v_columns IN
    SELECT
      ...
      (SELECT ' + (' || string_agg(format('sum(_timescaledb_functions.compressed_data_column_size(%I,NULL::%s))', attname, atttypid::regtype), ' + ' ORDER BY attnum) || ')' FROM pg_attribute att WHERE att.attrelid=i.indrelid AND attnum =ANY(i.indkey)),
      array_length(i.indkey,1) FROM pg_index i
    WHERE i.indrelid = v_uncompressed_chunk
  LOOP
    v_index_header := 8; -- Index tuple header

    EXECUTE format('SELECT ((%s * %s) %s) FROM %s', tuples, v_index_header, v_varlen_query, v_compressed_chunk) INTO v_index_size;
