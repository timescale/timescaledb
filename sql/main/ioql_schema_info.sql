CREATE OR REPLACE FUNCTION get_partition_replicas(epoch partition_epoch, replica_id SMALLINT)
    RETURNS SETOF partition_replica LANGUAGE SQL STABLE AS
$BODY$
SELECT DISTINCT pr.*
FROM partition p
...
CREATE OR REPLACE FUNCTION get_local_chunk_replica_node_for_pr_time_desc(pr partition_replica)
    RETURNS SETOF chunk_replica_node LANGUAGE SQL STABLE AS
$BODY$
SELECT crn.*
