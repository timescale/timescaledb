...
CREATE OR REPLACE FUNCTION _timescaledb_internal.get_quoted_column_names(
    hypertable_id INTEGER
)
...
CREATE OR REPLACE FUNCTION _timescaledb_internal.get_partition_for_epoch(
    epoch     _timescaledb_catalog.partition_epoch,
    key_value TEXT
)
    RETURNS _timescaledb_catalog.partition LANGUAGE PLPGSQL STABLE STRICT AS
$BODY$
DECLARE
...
-- TODO Only used in tests -- okay to put in _timescaledb_internal?
CREATE OR REPLACE FUNCTION get_open_partition_for_key(
    hypertable_id INTEGER,
    key_value     TEXT
)
    RETURNS _timescaledb_catalog.partition LANGUAGE SQL STABLE AS
$BODY$
    SELECT p.*
    FROM _timescaledb_catalog.partition_epoch pe,
         _timescaledb_internal.get_partition_for_epoch(pe, key_value) p
...
CREATE OR REPLACE FUNCTION _timescaledb_internal.time_col_name_for_crn(
    schema_name NAME,
    table_name  NAME
)
...
BEGIN
    ...
    INNER JOIN _timescaledb_catalog.chunk_replica_node crn ON (crn.chunk_id = c.id)
...
--
-- schema_name, table_name - name of the schema and table for the table represented by the crn.
CREATE OR REPLACE FUNCTION _timescaledb_internal.time_col_type_for_crn(
    schema_name NAME,
    table_name  NAME
)
