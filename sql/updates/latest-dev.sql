...
BEGIN
    ...
    LOOP
        ...
        json_entries := json_entries || jsonb_build_object('type', sparse_type, 'column', col_part);
    END LOOP;
...
BEGIN
    ...
    LOOP
        IF rec.compress_relid IS NULL THEN
            -- For main relations, use settings from highest chunk id
            SELECT comp_chunk.schema_name, comp_chunk.table_name
            INTO schema_name, table_name
            FROM _timescaledb_catalog.chunk chunk
            INNER JOIN _timescaledb_catalog.chunk comp_chunk
                ON chunk.compressed_chunk_id = comp_chunk.id
            ...
            WHERE hypertable.schema_name = n.nspname
                AND hypertable.table_name = c.relname
            ORDER BY chunk.compressed_chunk_id DESC
            LIMIT 1;
